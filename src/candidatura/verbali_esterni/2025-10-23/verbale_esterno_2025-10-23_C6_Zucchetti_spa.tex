...
\usepackage{graphicx}
% Percorsi per le immagini (assumendo che le risorse siano nelle cartelle superiori)
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textcolor{gray}{\term{Verbale} Riunione - Gruppo 17}}
\fancyfoot[C]{\thepage}
...
\begin{center}
    {\LARGE \textbf{\term{Verbale} Riunione Numero 1}}\\[0.2cm]
...
I server ospitano localmente vari modelli LLM (tra 7B e 27B) gestiti tramite \textbf{Ollama}, un \term{Framework} che permette l’accesso ai modelli via \textbf{API endpoint}.
...
\begin{itemize}
    ...
    \item Fornire due prompt distinti: uno di \textbf{sistema} (per definire lo stile o il ruolo dell’AI) e uno \textbf{\term{Utente}} (contenente il testo da elaborare).
\end{itemize}

Non è previsto il training o la progettazione di modelli personalizzati. La possibilità di sviluppare modelli non parametrici non è stata esclusa, ma non è un \term{Requisito} del progetto.

\subsection{Gestione utenti e \texorpdfstring{Architettura}{\term{Architettura}} applicativa}
È stato consigliato di implementare un sistema di \textbf{login} e un \term{Backend} di gestione utenti e note. Tuttavia, la gestione dei dati dovrà avvenire in locale per un singolo \term{Utente}, mantenendo il focus sull’aspetto personale dell’applicazione.
...
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.45\linewidth}|c|c|}
\hline
\rowcolor{gray!60}
...
\renewcommand{\arraystretch}{1.2}
\arrayrulecolor{black}
\begin{tabular}{|c|l|l|c|>{\raggedright\arraybackslash}p{0.35\linewidth}|}
\hline
\rowcolor{gray!60}
\textbf{Versione} & \textbf{Ruolo} & \textbf{Nome} & \textbf{Data e ora} & \textbf{Descrizione} \\
\hline
\rowcolor{white}
1.0.0 & Redatto da & Dennis Parolin & 2025-10-23 & Stesura iniziale del \term{Verbale} \\
\hline
\rowcolor{gray!20}
1.0.0 & Revisione & Gabriele Scaggiante & 2025-10-23 & Controllo approfondito del \term{Verbale} \\
\hline
\rowcolor{white}
1.0.0 & Conferma & Tutti i membri & 2025-10-28 & Conferma da parte di tutti del \term{Verbale} \\
\hline
\end{tabular}
\end{center}

\input{../spazio_firma.tex}
\end{document}
