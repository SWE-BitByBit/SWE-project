...
Il dominio tecnico del progetto \textit{“L’app che Protegge e Trasforma”} riguarda la progettazione e lo sviluppo di un’applicazione mobile multipiattaforma, sicura e scalabile, basata su \term{Architettura} \term{Cloud} e principi di \textit{security-by-design}. L’obiettivo tecnico è realizzare un sistema intelligente capace di rilevare, prevenire e gestire situazioni di rischio legate alla violenza di genere, garantendo privacy, affidabilità e semplicità d’uso.
Le principali aree tecnologiche coinvolte sono:

\begin{itemize}
\item \textbf{Sviluppo mobile:} sviluppo nativo cross-platform (iOS/Android) tramite \term{Framework} \textbf{Flutter}, per garantire coerenza, prestazioni elevate e manutenibilità.
\item \textbf{\term{Architettura} \term{Cloud} \term{Serverless}:} utilizzo di servizi \textbf{AWS} (API Gateway, Lambda, DynamoDB, RDS, S3) per realizzare un’infrastruttura scalabile, resiliente e a basso costo di gestione.
...
\item \textbf{Comunicazione tra front-end e back-end:} \term{API REST} sicure per l’interazione tra app mobile e \term{Backend}; gestione asincrona tramite \textbf{AWS SQS/Kinesis} e orchestrazione con \textbf{Step Functions}.
...
\subsubsection{Punti a favore}
\begin{itemize}
    \item Il progetto propone un’applicazione strutturata e completa, con una chiara suddivisione tra frontend e \term{Backend}.
...
\begin{itemize}
    ...
    \item Ci preoccupa la presenza di numerosi vincoli tecnici (\term{CI/CD}, coverage minimo, documentazione estesa). Preferiamo un progetto con pochi vincoli in modo da poterci focalizzare su pochi aspetti in maniera mirata.
    ...
    \item La complessità infrastrutturale (AWS, \term{GitHub Actions}, orchestrazione \term{Cloud}) potrebbe essere eccessiva per il nostro livello di esperienza attuale.
\end{itemize}
...
\begin{itemize}
    ...
    \item Il progetto offre la possibilità di lavorare su un’infrastruttura \term{Cloud} moderna, sperimentando concetti come microservizi, scalabilità e sicurezza.
\end{itemize}
...
\subsubsection{Descrizione}
Il \term{Capitolato} riguarda lo sviluppo di View4Life, una piattaforma per la gestione degli impianti domotici in residenze protette per anziani. Il progetto richiede la realizzazione di un applicativo web responsive e di un’infrastruttura \term{Cloud} containerizzata, con l’obiettivo di monitorare e controllare dispositivi smart come luci, tapparelle, termostati, sensori di presenza e caduta e attuatori per accessi e allarmi.
