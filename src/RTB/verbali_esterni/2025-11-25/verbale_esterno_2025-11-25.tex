...
\hline
\rowcolor{gray!20}
0.1.1 & 2026-03-05 & Dennis Parolin & Aggioranto il template del documento alla versione più recente & Ferdinando Fracasso \\
\hline
\rowcolor{white}
...
\section{Discussioni}
Durante la sessione, si è stabilito di adottare la metodologia del Design \term{Sprint} al fine di identificare con precisione il target di utenza finale e definire le funzionalità core da sviluppare. Successivamente, l'attività si è concentrata sulla realizzazione di mockup preliminari, volti a delineare l'identità visiva e l'\term{Architettura} dell'applicazione.
...
\begin{itemize}
    ...
    \item Rete di supporto (soggetti selezionati dall'\term{Utente} come contatti fidati).
\end{itemize}

A seguito di un confronto interno, si è deliberato di focalizzare il target primario sulle donne di età compresa tra i 18 e i 30 anni. Tuttavia, si è deciso di includere anche utenti esterni interessati all'argomento, prevedendo una sezione informativa accessibile sia a questi ultimi che all'\term{Utente} target principale.

\subsection{Analisi del Contesto d'Uso}
Il gruppo di lavoro ha successivamente analizzato il contesto in cui l'\term{Utente} potrebbe trovarsi durante l'utilizzo dell'applicazione.
È emerso che l'applicativo si configura principalmente come strumento di prevenzione volto a mitigare il degenerare di situazioni a rischio. Si presuppone, infatti, che in casi di pericolo imminente la vittima contatti direttamente le autorità competenti (es. 118), bypassando l'applicazione. Pertanto, lo scenario ideale prevede che l'\term{Utente} disponga di tempo sufficiente per interagire con l'app. Nonostante ciò, verrà garantito un supporto anche in situazioni di maggiore stress.
Nello specifico, l'installazione e l'utilizzo dell'applicazione sono previsti quando l'\term{Utente}:
...
\\Le funzionalità principali selezionate sono:
...
Definite le specifiche funzionali, si è proceduto alla valutazione dell'Interfaccia \term{Utente} (UI). Ciascun membro del team ha prodotto una bozza grafica su carta, successivamente presentata al gruppo, come illustrato in Figura \ref{fig:mockup}.
...
\hline
\rowcolor{white}
Definizione delle funzionalità operative da implementare & VE \ignoreglossary{RTB} 2.1 \\
\hline
\rowcolor{gray!20}
Definizione delle linee guida per l'Interfaccia \term{Utente} (UI) &  VE \ignoreglossary{RTB} 2.2 \\
\hline
\end{tabular}
\end{center}
...
\hline
\rowcolor{white}
Analisi tecnica preliminare dei servizi AWS per l'individuazione delle risorse idonee all'implementazione delle funzionalità & VE \ignoreglossary{RTB} 2.1 & \href{https://github.com/SWE-BitByBit/SWE-docs/issues/85}{\textbf{\#85}} \\
\hline
\rowcolor{gray!20}
Scelta e finalizzazione del design dell'Interfaccia \term{Utente} (UI) per il \term{Poc} & VE \ignoreglossary{RTB} 2.2 & \href{https://github.com/SWE-BitByBit/SWE-docs/issues/86}{\textbf{\#86}} \\
\hline
\end{tabular}
\end{center}

\textbf{Nota Metodologica:} Si precisa che l'attuale fase progettuale è focalizzata sull'\term{Analisi Dei Requisiti} e sullo studio di fattibilità tecnologica. L'effettiva fase di sviluppo software e programmazione sarà avviata esclusivamente a seguito del consolidamento delle scelte architetturali e dello stack tecnologico.
