...
\subsubsection{Scopo}
Il \term{Processo} primario di fornitura definisce le attività da intraprendere da parte del fornitore e l’analisi e pianificazione delle risorse necessarie per garantire l’efficienza e la conformità del progetto ai requisiti discussi con il \term{Proponente}.
...
La comunicazione avverrà principalmente tramite uno spazio su Google Chat messo a disposizione dal \term{Proponente} Miriade S.r.l., al fine di permettere una comunicazione più rapida in caso di eventuali dubbi o domande.
Inoltre il \term{Proponente} si rende disponibile a incontri da remoto, tramite Google Meet, o in presenza, quando necessario per la \term{Verifica} dello stato di avanzamento del progetto.
...
Tale attività produce il documento dell'\term{Analisi Dei Requisiti} che stabilisce le basi per un accordo tra la \term{Proponente} e il \textbf{gruppo BitByBit} su quello che il prodotto software è tenuto a fare.
\par L’\term{Analisi Dei Requisiti} obbliga tutte le parti coinvolte a valutare in modo completo e condiviso ciò che il sistema dovrà realizzare prima dell’avvio della progettazione. Questo riduce significativamente il rischio di dover intervenire successivamente su design, codice o test.
\par Il gruppo si impegna inoltre a monitorare con continuità l’allineamento tra requisiti implementati e requisiti concordati con la \term{Proponente}, aggiornandoli in modo controllato quando necessario.
...
\end{itemize}
  \item \textbf{\term{To-Do}}: presenta, sotto forma di tabella, gli incarichi operativi pianificati a seguito della riunione. Ogni \textbf{\term{To-Do}} è identificata da:
  \begin{itemize}
      \item una descrizione sintetica;
      \item Il codice della decisione da cui è stata creata (per ogni decisione \term{Ci} possono essere 0 o più attività);
