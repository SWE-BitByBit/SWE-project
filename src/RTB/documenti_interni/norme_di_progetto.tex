...
Il processo di verifica dello stile avviene su due livelli:
\begin{itemize}
    ...
    \item \textbf{Remoto}: la pipeline di \textbf{GitHub Actions} verifica la conformità del codice ad ogni \textit{push}. Come da policy, il mancato rispetto delle norme di formattazione impedisce tecnicamente il merge delle modifiche nel ramo principale.
\end{itemize}
...
Gli strumenti critici per il mantenimento dello standard, le cui specifiche tecniche verranno consolidate durante la Product Baseline (PB), sono classificabili in:
\begin{itemize}
...
\paragraph{Strumenti per la verifica documentazione}
Poiché il gruppo ha adottato la tecnica dell'\textbf{Ispezione} basata su checklist, la verifica della documentazione non richiede strumenti di automazione complessi, ma si avvale delle piattaforme di sviluppo e collaborazione:
\begin{itemize}
...
\begin{enumerate}
    \item \textbf{Test di Unità}: Verifica delle singole componenti (funzioni/classi) in isolamento.
    \begin{itemize}
...
\paragraph{Strumenti per la verifica del codice}
...
A differenza della verifica, che si configura come un controllo tecnico continuo sul codice, la validazione ha lo scopo di accertare che il sistema software realizzato dal gruppo \textit{BitByBit} sia pienamente conforme alle aspettative e ai bisogni reali dell'azienda proponente \textbf{Miriade}.
...
Questo approccio metodologico garantisce che non venga sviluppato codice non richiesto e, soprattutto, che ogni requisito concordato con \textbf{Miriade} sia verificabile tramite un test specifico. Un requisito privo di tracciamento verso un test è considerato non validabile e quindi non accettabile.
