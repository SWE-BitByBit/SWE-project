...
\hline
\rowcolor{gray!20}
1.0.1 & 2026-04-11 & Ferdinando Fracasso & Aggiunto termine "Presentation Logic" & Giovanni Visentin \\
\hline
\rowcolor{white}
...
\termdefinition{API REST}
Acronimo di \textit{Representational State Transfer}. Stile architetturale per sistemi distribuiti che definisce un set di vincoli e princìpi per la creazione di interfacce web, basato sullo scambio logico di messaggi senza stato tramite il protocollo HTTP.

\termdefinition{Architettura}
Organizzazione generale del sistema e dei suoi componenti.

\termdefinition{Architettura a Strati}
Modello architetturale (noto come \textit{Layered Architecture}) in cui i componenti del software sono disposti in livelli gerarchici isolati. Ogni strato ha una responsabilità specifica e comunica solitamente solo con gli strati adiacenti, favorendo la separazione degli intenti e la manutenibilità.
...
\termdefinition{Clean Architecture}
Modello di progettazione del software finalizzato alla creazione di sistemi scalabili, testabili e manutenibili. Separa il codice in fasce concentriche in cui la logica di business è totalmente isolata e indipendente da dettagli esterni quali database, framework e interfacce utente.

\termdefinition{Cloud}
Modello di erogazione di risorse informatiche (server, storage, database, calcolo) su richiesta tramite Internet, che svincola l'acquirente dalla necessità di possedere e manutenere hardware fisico in loco.
...
\termdefinition{Data Access Layer}
...
\termdefinition{Infrastructure as Code}
Prassi che consiste nel tracciamento e nel provisioning di data center e infrastrutture in cloud tramite file di testo leggibili dalla macchina (detti \textit{template}), anziché mediante l'uso di interfacce grafiche o configurazioni manuali.
...
\termdefinition{Model-View-ViewModel (MVVM)}
...
\termdefinition{Payload}
La porzione di dati utili e significativi trasportata all'interno di un messaggio scambiato in rete (come una richiesta HTTP), escludendo i metadati o le intestazioni tecniche (\textit{headers}) necessarie per formattare il pacchetto.
...
\termdefinition{Presentation Logic}
Insieme delle operazioni responsabili della rappresentazione grafica degli oggetti di dominio dell'applicazione.
Nel contesto dell'architettura Flutter adottata dal gruppo, la Presentation Logic risiede nel Data Layer, all'interno delle classi \texttt{ViewModel}.
...
\termdefinition{Scalabilità dinamica}
...
\termdefinition{Separation of Concerns}
...
\termdefinition{Single Point of Entry}
...
\termdefinition{Specifica Tecnica}
...
\termdefinition{State Management}
...
\termdefinition{YAML}
Acronimo ricorsivo per \textit{YAML Ain't Markup Language}. Linguaggio di rappresentazione per la serializzazione dei dati, leggibile universalmente dall'uomo e largamente utilizzato per la stesura di file di configurazione per logiche CI/CD o per infrastrutture.

% ==============================
\section{Z}

\termdefinition{Zero Ops}
Acronimo di \textit{Zero Operations}. Paradigma aziendale e logico in cui i team di sviluppo responsabili del prodotto sono sollevati da tutte le operazioni manuali di manutenzione, aggiornamento o allocazione dei servizi fisici che ospitano il sistema.
