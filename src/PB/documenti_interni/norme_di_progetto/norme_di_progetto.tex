...
\hline
\rowcolor{white}
1.2.0 & 2026-03-31 & Riccardo Manisi & Miglioramento del Processo di Verifica secondo standard ISO elencato. & Dennis Parolin \\
\hline
\rowcolor{gray!20}
...
\hline
\rowcolor{white}
0.11.2 & 2026-01-11 & Dennis Parolin & Aggiunti gli strumenti di supporto delle sezioni "Processo di Verifica", "Processo di Validazione" e "Codifica". Aggiunta la lista delle tabelle. & Riccardo Manisi \\
\hline
\rowcolor{gray!20}
0.11.1 & 2026-01-10 & Dennis Parolin & Sistemate le sezioni "Processo di Verifica" e "Processo di Validazione" tra i Processi di Supporto. Sistemata la sezione "Codifica" tra i Processi Primari. & Riccardo Manisi\\
\hline
\rowcolor{white}
0.11.0 & 2026-01-06 & Dennis Parolin & Aggiunta sezioni "Processo di Verifica" e "Processo di Validazione" tra i Processi di Supporto. Aggiunta la sezione "Codifica" tra i Processi Primari. & Riccardo Manisi \\
\hline
\rowcolor{gray!20}
...
La sua redazione si ispira, a scopo puramente informativo, allo standard ISO/IEC 12207:1995, dal quale vengono riprese alcune classificazioni dei Processo, senza assumerlo come riferimento normativo. In particolare, lo standard individua tre tipologie di Processo:
\begin{enumerate}
...
Il Processo primario di fornitura definisce le attività che devono essere 
intraprese dal fornitore. Il Processo consta dell’analisi e pianificazione delle risorse necessarie 
per garantire l’efficienza e la conformità del progetto ai requisiti discussi con la Proponente.
...
\begin{itemize}
    ...
    \item \textbf{Google Chat}: comunicazione asincrona con la Proponente.
...
La comunicazione avverrà principalmente tramite uno spazio su Google Chat messo a disposizione dal Proponente \textbf{Miriade S.r.l.}, al fine di permettere una comunicazione più rapida in caso di eventuali dubbi o domande.
Inoltre il Proponente si rende disponibile a incontri da remoto, tramite Google Meet, o in presenza, quando è opportuna la Verifica dello stato di avanzamento del progetto.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Documento in cui il gruppo \textbf{BitByBit} formalizza la propria candidatura al Capitolato dell’azienda \textbf{Miriade S.r.l.}.
    ...
    \begin{itemize}
        \item Il presente documento, avente l'obiettivo di definire i processi del Way of working e le norme che il gruppo \textbf{BitByBit} si impegna a seguire per l'intera durata del progetto.
    \end{itemize}
...
\\Il Processo di sviluppo deve seguire le pratiche del Processo di gestione dei processi (\hyperref[sec:gestione-processi]{vedi sezione~\ref*{sec:gestione-processi}}), stabilire l'infrastruttura secondo il Processo di infrastruttura (\hyperref[sec:infrastruttura]{vedi sezione~\ref*{sec:infrastruttura}}), e gestire il Processo a livello organizzativo tramite i processi di miglioramento (\hyperref[sec:processo-miglioramento]{vedi sezione~\ref*{sec:processo-miglioramento}}) e di formazione (\hyperref[sec:processo-formazione]{vedi sezione~\ref*{sec:processo-formazione}}).
...
\begin{itemize}
    ...
    \item \textbf{Flutter}: Framework utilizzato per lo sviluppo dell'interfaccia Utente dell'applicazione mobile.
    \item \textbf{Visual Studio Code}: IDE adottato dal gruppo come ambiente di sviluppo comune, configurato 
    ...
    il tracciamento delle Issue e la revisione del codice tramite Pull Request.
    \item \textbf{GitHub Actions}: piattaforma di CI/CD utilizzata per 
    automatizzare i controlli di qualità ad ogni \textit{push}. Esegue la formattazione automatica del codice, 
    l'Analisi Statica tramite SonarQube e blocca il Merge nel ramo principale in caso di esito negativo.
    \item \textbf{AWS SAM}: Framework open source utilizzato per la build e il deploy dei servizi AWS. Consente 
    di definire l'infrastruttura Serverless tramite file di configurazione e di automatizzarne il rilascio tramite la pipeline di CI/CD.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Analisi dei requisiti di sistema}, l'analisi del sistema da implementare deve essere condotta in modo dettagliato, al fine di produrre una descrizione delle funzionalità e delle capacità del sistema, nonché dei requisiti Utente e di sistema.
    \item \textbf{Progettazione dell'Architettura}, deve essere identificata un'Architettura di alto livello per identificare gli elementi software. Deve essere garantito che tutti i requisiti siano allocati tra gli elementi.
...
L'attività di Analisi dei requisiti ha l’obiettivo di trasformare i requisiti Utente espressi nel Capitolato, nel nostro caso il \textbf{Capitolato C4} proposto da \textbf{Miriade S.r.l.}, in requisiti di sistema chiari, completi e verificabili. 
Tale attività produce il documento dell'Analisi dei requisiti che stabilisce le basi per un accordo tra la Proponente e il fornitore sulle funzionalità che verranno implementate nel prodotto..
...
Dove:
\begin{itemize}
    \item \texttt{R}: è per abbreviare la parola Requisito
    \item \texttt{<tipo>}: è la sigla che indica la tipologia di Requisito
    \begin{itemize}
        \item \texttt{F}: per indicare che è un Requisito funzionale
        \item \texttt{Q}: per indicare che è un Requisito di Qualità
        \item \texttt{V}: per indicare che è un Requisito di Vincolo
    \end{itemize}
    \item \texttt{<importanza>}: è la sigla a due lettere che indica la categoria del Requisito
...
\begin{itemize}
    ...
    \item \texttt{Service}: classi del Data Layer responsabili della Business Logic (es. \texttt{AuthService}).
\end{itemize}
...
\begin{itemize}
    \item \textbf{Locale}: l'IDE adottato dal gruppo è configurato per eseguire \texttt{black} al salvataggio del file tramite l'estensione ufficiale.
    \item \textbf{Remoto}: la pipeline di GitHub Actions per eseguire la Ci applica la formattazione Python ad ogni \textit{push}, impedendo il Merge di codice non formattato correttamente nel Branch principale.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item \texttt{service/}, contiene l'implementazione della Business Logic core del microservizio;
            \item \texttt{repository/}, contiene gli adattatori per l'accesso fisico ai dati (es. implementazioni per S3 o DynamoDB);
            \item \texttt{ports/}, contiene le interfacce astratte (Port) che definiscono i contratti per i Repository e i servizi, garantendo il disaccoppiamento;
...
Ogni azione che porta a dei cambiamenti ad un documento del gruppo deve iniziare con l’apertura di una Issue su GitHub per permettere la tracciabilità nella board del GitHub Project. Ogni commit deve fare riferimento ad una sola azione in modo da garantire che il Processo iterativo di revisione ed eventuali correzioni di errori sia il più veloce possibile.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item i \textbf{verificatori}, che identificano i membri che hanno eseguito attività di Verifica per quel documento; 
...
I verbali rappresentano i rendiconti di riunioni ufficiali interne, a cui partecipano solo i componenti del gruppo, ed esterne, a cui invece partecipano anche enti esterni al gruppo come impiegati dell'azienda Proponente del Capitolato o i professori del corso. Ogni Verbale oltre alle sezioni precedentemente elencate deve presentare al suo interno i seguenti elementi nell'ordine in cui sono descritti:
\begin{enumerate}
...
\end{itemize}
  \item \textbf{To-Do}: presenta, sotto forma di tabella, gli incarichi operativi pianificati a seguito della riunione. Ogni \textbf{To-Do} è identificato da:
  \begin{itemize}
      ...
      \item il numero della Issue GitHub di quella attività presente nel GitHub Project.
  \end{itemize}
\end{enumerate}
...
\paragraph{\ignoreglossary{Verifica}} Il verificatore esamina sia la correttezza formale sia sostanziale del documento.
    \begin{itemize}
        \item Se le modifiche risultano corrette, il verificatore approva la Pull Request.
...
\paragraph{Approvazione e \ignoreglossary{Pull Request}} Una volta ottenuta l’approvazione del verificatore, la Pull Request viene sottoposta alla visione del responsabile, che effettua un ultima revisione formale. Solo il responsabile è autorizzato a procedere con il Merge della Pull Request nel Branch \texttt{develop}.
...
\begin{itemize}
    ...
    \item \textbf{controllo della configurazione}, metodologie per stabilire l'identificazione, l'approvazione o rifiuto, la Verifica e l'eventuale rilascio delle richieste di modifica; 
...
La pianificazione complessiva è supportata dalle GitHub Project Board, che permettono di consultare Product backlog e Sprint backlog tramite viste personalizzate, semplificando la gestione dei carichi di lavoro e l’avanzamento delle attività.
...
Quando la Pull Request viene approvata dal verificatore, il responsabile esegue la revisione. Si impegna ad eseguire il Merge nel Branch \texttt{develop}. A questo punto, la modifica entra ufficialmente in Baseline e diventa parte del progetto.
...
% ==== SEZIONE Processo DI \ignoreglossary{Verifica} ====
\subsection{Processo di \ignoreglossary{Verifica}} \label{sec:processo-verifica}
Il Processo di Verifica è l'insieme delle attività volte ad accertare che ogni prodotto del Ciclo di vita (codice, documenti, Architettura) sia conforme ai requisiti e agli standard di qualità stabiliti.
...
\begin{itemize}
    \item \textbf{SonarQube}: piattaforma di Analisi Statica per la rilevazione di bug, 
    vulnerabilità di sicurezza e code smell. Fornisce una serie di metriche già consolidate e la visualizzazione tramite indicatori dei report dei Test di Unità.
    I risultati sono consultabili tramite dashboard web, che fornisce una visione aggregata dello stato del codice nel tempo.
    \item \textbf{GitHub}: tramite l'interfaccia delle Pull Request permette ai verificatori di commentare righe specifiche del documento, richiedere modifiche e approvare il lavoro solo quando la Checklist di Verifica è soddisfatta.
...
\begin{itemize}
    \item \textbf{Implementazione di Processo}, si stabilisce il grado di Verifica e lo sforzo che deve essere svolto affinché questo 
    ...
    \item \textbf{Attività di Verifica}, insieme di verifiche condotte sistematicamente durante il Ciclo di vita del progetto per 
...
\subsubsection{Implementazione di processo}
Il gruppo \textit{BitByBit} integra la Verifica direttamente nel flusso di sviluppo quotidiano (Git Flow), adottando la politica del \textbf{Continuous Verification}.
Le regole operative sono le seguenti:
\begin{itemize}
    \item \textbf{Policy "No Verify, No Merge"}: nessun artefatto può essere integrato nel ramo di sviluppo principale (\texttt{develop}) se non ha superato con successo i controlli di Verifica.
    \item \textbf{Pull Request come Quality Gate}: ogni modifica deve essere proposta tramite Pull Request. In questa sede avviene la Verifica (automatica o manuale) e solo l'esito positivo autorizza il Merge.
...
La Verifica del software si articola in due momenti distinti: l'analisi del codice sorgente (statica) e l'esecuzione dei test (dinamica).

\paragraph{\ignoreglossary{Analisi Statica}} \label{par:analisi-statica}
Prima di essere eseguito, il codice viene analizzato da strumenti automatici integrati nella pipeline di CI/CD.
...
\begin{itemize}
    ...
    \item il valore delle metriche di qualità definite nel Piano di qualifica, ottenendo un riscontro immediato prima che gli errori vengano introdotti nella Code Base.
\end{itemize}
L'Analisi Statica viene eseguita su ogni \textit{push} nella pipeline di CI/CD: un esito negativo blocca il Merge nel 
...
SonarQube viene eseguito nella pipeline di CI/CD tramite GitHub Actions.

\paragraph{\ignoreglossary{Analisi Dinamica} (Testing)}
L'Analisi Dinamica Verifica il comportamento del software tramite la sua esecuzione.
...
\begin{enumerate}
    \item \textbf{Test di Unità}: Verifica delle singole componenti (funzioni/classi) in isolamento, tramite \texttt{flutter\_test} per il codice Dart e \texttt{pytest} per il codice Python.
    \begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Test di Integrazione}: Verifica della collaborazione tra unità già testate.
    \item \textbf{Test di Integrazione}: Verifica della collaborazione tra unità già testate.
    \begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Test di Sistema}: Verifica del sistema completo rispetto ai requisiti funzionali e prestazionali mappati nell'Analisi dei requisiti.
    \item \textbf{Test di Regressione}: riesecuzione automatica della suite di test ad ogni \textit{push} tramite GitHub Actions, per garantire che le nuove modifiche non compromettano funzionalità preesistenti. I risultati di copertura vengono raccolti da SonarQube e sono consultabili tramite la sua dashboard.
    \item \textbf{Test di Accettazione}: verifiche formali condotte con la Proponente per confermare che il software soddisfi i requisiti concordati prima del rilascio.
\end{enumerate}

\subsubsection{Classificazione dei Test}
Per garantire l'ordine nel \textbf{Piano di qualifica}, il gruppo adotta una nomenclatura univoca per il tracciamento:
\begin{center}
    \texttt{T-[ID]-[Tipo]}
\end{center}
...
% ==== SEZIONE Processo DI Validazione ====
\subsection{Processo di Validazione}
Il Processo di Validazione costituisce l'ultimo livello di controllo qualitativo prima del rilascio formale del prodotto.
A differenza della Verifica, che si configura come un controllo tecnico continuo sul codice, la Validazione ha lo scopo di accertare che il sistema software realizzato dal gruppo \textit{BitByBit} sia pienamente conforme alle aspettative e ai bisogni reali dell'azienda Proponente \textbf{Miriade S.r.l}.
...
L'attività prevede di mappare univocamente ogni Requisito (funzionale, di qualità o vincolo) ai relativi test di Validazione.
Questo approccio metodologico garantisce che non venga sviluppato codice non richiesto e, soprattutto, che ogni Requisito concordato con \textbf{Miriade S.r.l} sia verificabile tramite un test specifico. Un Requisito privo di tracciamento verso un test è considerato non validabile e quindi non accettabile.

\subsubsection{Esecuzione dei Test Di Accettazione}
La Validazione si concretizza con l'esecuzione dei \textbf{Test di Accettazione} (UAT - User Acceptance Testing).
In questa fase il gruppo non si limita a verificare l'assenza di errori, ma simula scenari di utilizzo reali (User Scenarios) per valutare il comportamento del sistema dal punto di vista dell'Utente finale.
...
Analogamente alla Verifica, anche il Processo di Validazione si avvale di strumenti specifici per gestire la complessità del tracciamento e l'esecuzione dei test.
...
\begin{itemize}
    \item \textbf{Ambiente di Deployment}: i test verranno eseguiti su un'istanza del software deployata in un ambiente il più possibile simile a quello di produzione, accessibile tramite Browser Web standard (Chrome/Firefox).
    \item \textbf{GitHub Issues}: qualsiasi anomalia o comportamento inatteso riscontrato durante la Validazione (test fallito) verrà tracciato aprendo una Issue dedicata su GitHub, etichettata specificamente come bug di Validazione per distinguerla dai difetti tecnici di sviluppo.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Revisione e valutazione}, perché un prodotto entri in Baseline, le modifiche apportate devono essere prima verificate da un verificatore (\hyperref[sec:processo-verifica]{si veda Sezione~\ref*{sec:processo-verifica}}) e successivamente revisionate dal responsabile. Quest’ultimo accerta che i prodotti software e i piani risultino conformi ai requisiti stabiliti, valutando l’esito dei compiti completati e garantendo qualità e coerenza con gli obiettivi del Processo.
...
\hline
\rowcolor{white}
\centering Responsabile & È la figura che nel momento governa il team. Identifica le attività che sono state eseguite nello Sprint terminato e quelle che non sono state portate a termine. Durante l'evento di Sprint Planning stabilisce ed inserisce nello Sprint backlog le attività da eseguire nello Sprint successivo, individuando i costi, i rischi e i membri del gruppo in base ai loro ruoli. Rappresenta il gruppo all'esterno, interfacciandosi con gli enti esterni quali i professori e l'azienda Proponente.    \\
\hline
\rowcolor{gray!20}
...
Le figure che interessano le Pull Request sono principalmente due:
\begin{itemize}
...
\end{itemize}
Questa struttura garantisce che ogni modifica venga validata da più persone e che il Processo rispetti il Processo di qualità (\hyperref[sec:accertamento-qualita]{si veda Sezione~\ref*{sec:accertamento-qualita}}).
...
\begin{itemize}
    ...
    \begin{itemize}
        \item uno a fine Sprint, comprendente Sprint Review Sprint Retrospective e Sprint Planning. In questa sede il \textbf{Responsabile} Verifica lo stato delle attività concluse, analizza eventuali problemi riscontrati e pianifica il lavoro del successivo Sprint;
        \item due durante lo svolgimento dello Sprint, in cui vengono trattati i temi dell'ordine del giorno e ciascun membro espone le attività completate, quelle ancora in corso e le difficoltà incontrate.
    \end{itemize}
...
\end{itemize}
L'esito di ogni incontro, interno o esterno, deve essere registrato mediante la redazione di un Verbale, secondo le modalità previste dal Processo di documentazione (\hyperref[sec:documentazione]{vedi Sezione~\ref*{sec:documentazione}}). 
...
\begin{itemize}
    ...
    \item \textbf{Formula:} $EV=\frac{\text{\# di Issue completate}}{\text{\# di Issue pianificate}}\cdot PV$
...
\begin{itemize}
    ...
    \item \textbf{Formula:} $\frac{\text{\# Issue completate nello Sprint}}{\text{\# Issue pianificate}} \cdot 100$
