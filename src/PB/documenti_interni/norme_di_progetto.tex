...
\begin{itemize}
    \item \textbf{Classi, enum, typedef ed estensioni}: si utilizza \texttt{UpperCamelCase} (es. \texttt{UserRepository}, \texttt{AuthService}).
    \item \textbf{Variabili, parametri, metodi e costanti}: si utilizza \texttt{lowerCamelCase} (es. \texttt{userName}, \texttt{fetchData}).
    \item \textbf{File, directory e prefissi di import}: si utilizza \texttt{lowercase\_with\_underscores} (es. \texttt{user\_repository.dart}, \texttt{auth\_service.dart}).
    \item \textbf{Lingua codice}: nomi di classi, variabili, metodi e file devono essere in \textbf{lingua inglese}.
    \item \textbf{Lingua commenti}: i commenti nel codice devono essere redatti in \textbf{lingua italiana}.
    \item \textbf{Commenti di documentazione}: ogni metodo deve essere documentato tramite un commento (usando il comando \texttt{///}).
...
La formattazione del codice Dart viene delegata al tool ufficiale \texttt{dart format}, che applica automaticamente le regole stilistiche definite da \textit{Dart}. 
...
\begin{itemize}
    \item \textbf{Locale}: l'IDE adottato dal gruppo (Visual Studio Code) è configurato per eseguire \texttt{dart format} al salvataggio del file.
...
\subparagraph{Struttura della repository frontend} \label{subpar:struttura-repo-frontend}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{ui/} — contiene tutto il codice relativo alla presentazione, organizzato per funzionalità. Per ciascuna funzionalità sono presenti:
        \begin{itemize}
            \item file \textit{View} (widget Flutter che descrivono l'interfaccia utente);
            \item file \textit{ViewModel} (classi Dart che gestiscono lo stato e la logica della view, una per ogni view).
        \end{itemize}
        \item \texttt{domain/} — contiene i modelli di dominio dell'applicazione, ovvero le classi che rappresentano i dati nel formato consumato dal layer \texttt{ui}. Non dipende da \texttt{data} né da \texttt{ui}.
        \item \texttt{data/} — contiene il layer di accesso ai dati, suddiviso in:
        \begin{itemize}
            \item \textit{Repository}: classi che rappresentano la fonte di verità per un tipo di dato; si occupano di recuperare, trasformare e memorizzare i dati provenienti dai \textit{Service};
            \item \textit{Service}: classi che comunicano con sorgenti dati esterne (API REST, database locali, ecc.).
        \end{itemize}
    \end{itemize}
    \item \texttt{test/} — contiene i test unitari del progetto. La struttura interna rispecchia quella di \texttt{lib/}, in modo che ogni file sorgente abbia il corrispondente file di test con suffisso \texttt{\_test.dart}.
    \item \texttt{testing/} — sotto-pacchetto dedicato ai mock e alle utilities condivise tra i test. Contiene le implementazioni fittizie delle classi (mock di \textit{Repository} e \textit{Service}) che i file in \texttt{test/} devono utilizzare, evitando dipendenze da servizi reali durante l'esecuzione dei test.
\end{itemize}
