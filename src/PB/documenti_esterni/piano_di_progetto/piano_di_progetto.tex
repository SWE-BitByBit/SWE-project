...
\newcommand{\VersioneAttuale}{1.1.1}
...
Hai fatto benissimo a ricontrollare! Effettivamente il nuovo PDF che hai caricato riporta dei dati molto diversi per il consuntivo: le ore lavorate sono 63 (contro le 48 del file precedente) e tutti i membri del gruppo hanno partecipato attivamente, allineandosi quasi perfettamente con il preventivo.

Questo significa che anche il testo descrittivo relativo ai "Rischi incontrati" e alla "Retrospettiva" doveva essere adattato, dato che non c'è più il problema dell'assenza di un componente (RO4), ma anzi il gruppo ha lavorato in modo molto coeso e produttivo.

Ecco il codice LaTeX aggiornato con i nuovi calcoli (tabelle, delta (+/-), budget rimanente e grafici a torta sistemati con le nuove angolazioni) e i testi revisionati per riflettere questo ottimo andamento. Puoi sostituire tutto il blocco dello Sprint 8 con questo:
Snippet di codice
...
\paragraph{Informazioni generali e attività da svolgere}
Lo \term{Sprint} 8 segna l'inizio dei lavori per la Product Baseline (\term{PB}), spostando il focus sull'impostazione architetturale dell'MVP, la ripianificazione a lungo termine e l'aggiornamento della documentazione a seguito dei feedback ricevuti dai docenti (\term{Prof. Vardanega} e \term{Prof. Cardin}).
Le attività pianificate e svolte includono:
\begin{itemize}
    \item \textbf{Gestione (Responsabile):} Creazione delle macro-issue nel project GitHub e conseguente aggiornamento del diagramma di Gantt. Definizione della nuova \term{Milestone} per la \term{PB} con i relativi artefatti concordati dal gruppo. Miglioramento del file Excel per la rendicontazione delle ore. Redazione dei verbali degli incontri con la \term{Proponente} Miriade (12 e 30 marzo), resisi necessari per incrementare la comunicazione come suggerito dal docente, e relativa creazione di issue. Correzione di alcuni appunti documentali (es. rimozione numerazione sezioni dall'indice del Glossario e aggiornamento date nel PdP).
    \item \textbf{Amministrazione:} Setup della \term{Repository} per l'MVP (progetto Flutter, directory sorgente/test, progetto AWS SAM). Configurazione delle pipeline di \term{CI/CD} tramite GitHub Actions per il deploy su AWS e integrazione di \term{SonarQube Cloud} per l'analisi statica di Python e Dart (verificando il supporto per quest'ultimo). Aggiornamento corposo delle \textit{Norme di Progetto} con convenzioni di scrittura codice, linee guida architetturali per Flutter e documentazione del processo di sviluppo sulla wiki. Correzioni strutturali sui processi (gerarchia processo-attività-procedure-strumenti) e sulla veste del sito web (es. riposizionamento della Lettera di Presentazione, rimozione etichette "File pdf").
    \item \textbf{Analisi:} Revisione e ampliamento dell'\textit{\term{Analisi dei Requisiti}} in risposta ai feedback ricevuti. Modifica dei diagrammi UML, aggiornamento del tracciamento requisiti e in particolare l'ampliamento dell'UC48 (Consultazione chatbot), precedentemente giudicato troppo generico.
    \item \textbf{Progettazione:} Creazione del documento \textit{Specifica Tecnica} (scopo, tecnologie impiegate). Inizio della progettazione dell'\term{Architettura} logica e di deployment, con focus sulle funzionalità primarie (chatbot, diario criptato/fittizio, contatti fidati). Realizzazione di bozze su Figma per l'aspetto dell'app. Creazione di \textit{Makefile} per l'esecuzione agevolata dei test.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[scale=1.2]
        % Responsabile: 14.6% -> 52.6°
        \draw[thick,fill=pieResp] (0,0) -- (0:1.5) arc (0:52.6:1.5) -- cycle;
        \draw (26.3:1.95) node {14.6\%};
        
        % Amministratore: 24.8% -> 89.3° (Cumulato: 141.8°)
        \draw[thick,fill=pieAmm] (0,0) -- (52.6:1.5) arc (52.6:141.8:1.5) -- cycle;
        \draw (97.2:1.85) node {24.8\%};

        % Analista: 0.7% -> 2.5° (Cumulato: 144.4°)
        \draw[thick,fill=pieAnal] (0,0) -- (141.8:1.5) arc (141.8:144.4:1.5) -- cycle;
        \draw (143:2.1) node {0.7\%};
        
        % Progettista: 51.1% -> 184° (Cumulato: 328.3°)
        \draw[thick,fill=pieDes] (0,0) -- (144.4:1.5) arc (144.4:328.3:1.5) -- cycle;
        \draw (236.3:1.8) node {51.1\%};

        % Verificatore: 8.8% -> 31.7° (Cumulato: 360°)
        \draw[thick,fill=pieVer] (0,0) -- (328.3:1.5) arc (328.3:360:1.5) -- cycle;
        \draw (344.1:1.95) node {8.8\%};
...
\paragraph{Consuntivo}
Di seguito il resoconto delle ore effettivamente impiegate. Il gruppo ha rispettato in modo estremamente fedele il piano orario previsto (63 ore su 68.5 stimate), con minimi scostamenti fisiologici nei ruoli di Amministratore e Progettista:
...
Tuttavia, con l'avvio della configurazione delle \term{GitHub Actions} per il deploy effettivo su AWS, si è parzialmente concretizzato il rischio \textbf{RO5 (Dipendenza da permessi esterni)}: i test di rilascio completi sono stati messi momentaneamente in attesa per permettere un'ulteriore verifica con la \term{Proponente} circa le autorizzazioni \term{IAM} dell'account.
...
La definizione precisa delle \textit{macro-issue} tramite diagramma di Gantt ha permesso di stabilire scadenze chiare: l'infrastruttura normativa è conclusa, mentre il documento di \textit{Specifica Tecnica} dovrà essere completato entro lo \term{Sprint} 9 per lanciare la fase di vera programmazione dell'MVP. 
Per ovviare ai blocchi legati all'ambiente \term{Cloud} reale (\textbf{RO5}), il gruppo continuerà a sviluppare e testare localmente sfruttando i container Docker (\textit{dynamodb-local} e Minio per S3), mantenendo un ritmo elevato in vista della \term{Product Baseline}.
\end{document}
