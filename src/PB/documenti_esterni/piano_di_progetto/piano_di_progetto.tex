...
\endhead
\rowcolor{white}
1.1.5 & 2026-05-05 & Ferdinando Fracasso & Scritta sezione \term{Sprint} 12 & Dennis Parolin \\
\hline
\rowcolor{gray!20}
1.1.4 & 2026-04-28 & Dennis Parolin & Scritta sezione \term{Sprint} 11 e aggiornata sezione \hyperref[subsec:attivita_previste_pb]{Attività previste per la Product Baseline (PB)} & Giovanni Visentin \\
\hline
\rowcolor{white}
1.1.3 & 2026-04-27 & Giovanni Visentin & Consuntivo \term{Sprint} 10 & Dennis Parolin \\
\hline
\rowcolor{gray!20}
1.1.2 & 2026-04-14 & Gabriele Scaggiante & Scritto il consuntivo dello \term{Sprint} 9 & Ferdinando Fracasso \\
\hline
\rowcolor{white}
1.1.1 & 2026-04-11 & Marco Sanguin & Consuntivo \term{Sprint} 8 & Gabriele Scaggiante \\
\hline
\rowcolor{gray!20}
1.1.0 & 2026-04-06 & Marco Sanguin & Iniziato riepilogo \term{Sprint} 8 & Dennis Parolin \\
\hline
\rowcolor{white}
...
Di seguito sono elencate le principali attività pianificate per il periodo della Product \term{Baseline}:
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|L|Z|}
        ...
        \hline
        \rowcolor{tableLineOdd}
        Redazione \term{Specifica Tecnica} & Documento che descrive l'\term{Architettura} del sistema, i diagrammi delle classi e le scelte progettuali effettuate per le componenti \term{Backend} e Frontend. Spiega inoltre come le varie classi sono collegate e comunicano tra loro. \\
        \hline
        \rowcolor{tableLineEven}
        Redazione Manuale \term{Utente} & Guida destinata all'\term{Utente} finale per illustrare le funzionalità dell'applicazione, le modalità di utilizzo e le procedure per la risoluzione di eventuali problematiche. \\
        \hline
        \rowcolor{tableLineOdd}
        Completamento \term{Piano di progetto} & Aggiornamento del documento con la descrizione di tutti gli \term{Sprint} pianificati per il periodo della \term{PB}. \\
        \hline
        \rowcolor{tableLineEven}
        ...
        \hline
        \rowcolor{tableLineOdd}
        Completamento e correzione \term{Piano di qualifica} & Revisione delle metriche di qualità definite nella \term{RTB} per correggere eventuali errori e definizione delle sezioni dedicate ai test da implementare per il codice sviluppato. \\
        \hline
        \rowcolor{tableLineEven}
        Ampliamento Glossario & Inserimento dei nuovi termini tecnici e degli acronimi emersi durante la fase di sviluppo della Product \term{Baseline} per garantire chiarezza terminologica. \\
        \hline
        \rowcolor{tableLineOdd}
        Realizzazione del Minimum Viable Product (\term{MVP}) & Sviluppo effettivo del codice per le componenti Frontend e \term{Backend}, comprensivo della realizzazione dei relativi test per assicurarne il corretto funzionamento. \\
        \hline
    \end{tabularx}
    \caption{Attività previste per la \term{PB}}
...
\begin{itemize}
    ...
    \item {\sloppy \textit{Documentazione:} Avvio della stesura dei documenti \textit{\term{Analisi dei requisiti}} (AdR), \textit{Norme di Progetto} (NdP) e \textit{\term{Piano di progetto}} (PdP). \par}
...
\begin{itemize}
    ...
    \item {\sloppy \textbf{Documentazione:} Avanzamento nel completamento dei diagrammi UML dei casi d'uso nell' \textit{\term{Analisi dei requisiti}} (AdR); prima redazione del \textit{\term{Piano di qualifica}} (PdQ) e aggiornamento delle \textit{Norme di Progetto} (NdP). \par}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|>{\columncolor{tableHeader}\bfseries}l|X|>{\columncolor{tableHeader}\bfseries}l|X|}
        ...
        \hline
    \end{tabularx}
    \caption{Riepilogo temporale \term{Sprint} 8}
\end{table}

\paragraph{Informazioni generali e attività da svolgere}
Lo \term{Sprint} 8 segna l'inizio dei lavori per la Product \term{Baseline} (\term{PB}), spostando il focus sull'impostazione architetturale dell'\term{MVP}, la ripianificazione a lungo termine e l'aggiornamento della documentazione a seguito dei feedback ricevuti dai docenti (Prof. Vardanega e Prof. Cardin) in seguito alla revisione \term{RTB}.
Le attività pianificate e svolte includono:
\begin{itemize}
    \item {\sloppy \textbf{Amministrazione:} Setup della \term{Repository} per l'\term{MVP} (progetto Flutter, directory sorgente/test, progetto AWS SAM). Configurazione delle pipeline di \term{CI/CD} tramite \term{GitHub Actions} per il deploy su AWS e integrazione di SonarQube \ignoreglossary{Cloud} per l'\term{Analisi Statica} di Python e Dart (verificando il supporto per quest'ultimo). Aggiornamento corposo delle \textit{Norme di Progetto} con convenzioni di scrittura codice, linee guida architetturali per Flutter e documentazione del \term{Processo} di sviluppo sulla wiki. Correzioni strutturali sui processi (gerarchia \term{Processo}-attività-procedure-strumenti) e sulla veste del sito web (es. riposizionamento della Lettera di Presentazione, rimozione etichette "File pdf"). \par}
    \item \textbf{Analisi:} Revisione e ampliamento dell'\textit{\term{Analisi dei requisiti}} in risposta ai feedback ricevuti. Modifica dei diagrammi UML, aggiornamento del tracciamento requisiti e in particolare l'ampliamento dell'UC48 (Consultazione chatbot), precedentemente giudicato troppo generico.
    \item \textbf{Progettazione:} Creazione del documento \textit{\term{Specifica Tecnica}} (scopo, tecnologie impiegate). Inizio della progettazione dell'\term{Architettura} logica e di \term{Deployment}, con focus sulle funzionalità primarie (chatbot, diario criptato/fittizio, contatti fidati). Realizzazione di bozze su Figma per l'aspetto dell'app.
\end{itemize}

\paragraph{Rischi attesi}
Tra i principali rischi identificati per questo periodo pre-implementativo troviamo:
\begin{itemize}
    \item \textbf{RT1 (Inesperienza tecnologica)}: Possibili ostacoli nell'integrazione di SonarQube con il codice Dart (essendo il supporto non nativo/ufficiale) e configurazioni errate nelle \term{GitHub Actions} per la \term{CI/CD}.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|C|C|C|C|C|C|}
        ...
        \hline
    \end{tabularx}
    \caption{Preventivo orario per componente - \term{Sprint} 8}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Distribuzione ore per ruolo - Preventivo \term{Sprint} 8}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|C|C|C|C|C|C|}
        ...
        \hline
    \end{tabularx}
    \caption{Consuntivo orario per componente - \term{Sprint} 8}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|c|c|c|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|}
        ...
        \hline
    \end{tabularx}
    \caption{\term{Sprint} 8 - Variazione nelle risorse disponibili}
\end{table}

\paragraph{Rischi incontrati}
Durante lo \term{Sprint} l'integrazione delle correzioni richieste dal Prof. Vardanega e dal Prof. Cardin ha comportato uno sforzo organizzativo notevole. Nonostante questo, il rischio \textbf{RO1 (Errata stima dei tempi)} è stato arginato egregiamente, comportando solo un lieve sforzo in più nel ruolo di Amministratore e Analista. 
Tuttavia, con l'avvio della configurazione delle \term{GitHub Actions} per il deploy effettivo su AWS, si è parzialmente concretizzato il rischio \textbf{RO5 (Dipendenza da permessi esterni)}: i test di rilascio completi sono stati messi momentaneamente in attesa per permettere un'ulteriore \term{Verifica} con la \term{Proponente} circa le autorizzazioni IAM dell'account.

\paragraph{Retrospettiva}
La retrospettiva evidenzia un eccellente livello di partecipazione e produttività dell'intero gruppo. Il bilancio delle ore lavorate (63 svolte contro le 68.5 pianificate) dimostra una ritrovata affidabilità nelle stime e nella capacità di esecuzione, lasciando alle spalle le difficoltà degli \term{Sprint} precedenti. L'infrastruttura dell'\term{MVP} è stata configurata con successo, l'integrazione con SonarQube è operativa e la gestione documentale è stata stabilizzata e validata in base ai feedback.
La definizione precisa delle \textit{macro-\term{Issue}} tramite diagramma di Gantt ha permesso di stabilire scadenze chiare: l'infrastruttura normativa è conclusa, mentre il documento di \textit{\term{Specifica Tecnica}} dovrà essere completato entro lo \term{Sprint} 9 per lanciare la fase di vera programmazione dell'\term{MVP}. 
Per ovviare ai blocchi legati all'ambiente \term{Cloud} reale (\textbf{RO5}), il gruppo continuerà a sviluppare e testare localmente sfruttando i container Docker (\textit{dynamodb-local} e Minio per S3), mantenendo un ritmo elevato in vista della Product \term{Baseline}.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|>{\columncolor{tableHeader}\bfseries}l|X|>{\columncolor{tableHeader}\bfseries}l|X|}
        ...
        \hline
    \end{tabularx}
    \caption{Riepilogo temporale \term{Sprint} 9}
\end{table}

\paragraph{Informazioni generali e attività da svolgere}
Lo \term{Sprint} 9 è orientato al completamento del documento di \term{Specifica Tecnica}, portandolo a un livello di dettaglio tale da consentire il pieno avvio della fase di codifica. In continuità con i progressi conseguiti nello \term{Sprint} 8, lo \term{Sprint} 9 si propone inoltre di avviare lo sviluppo lato frontend di alcune schermate dell’applicazione. Le attività pianificate includono:

\begin{itemize}
\item \textbf{Amministrazione:} Completamento e consolidamento delle attività pregresse relative alla definizione dei \term{Test di Integrazione}. Introduzione e aggiornamento di strumenti di monitoraggio del \term{Processo} tramite fogli condivisi, includendo la ridefinizione delle metriche di \term{Processo} precedentemente definite e la creazione di tabelle per il tracciamento manuale dei requisiti implementati durante gli \term{Sprint}.

\item \textbf{Progettazione:} Completamento e ampliamento della progettazione architetturale e di dettaglio attraverso la realizzazione dei diagrammi delle classi relativi alle diverse funzionalità del sistema. Definizione e rappresentazione dell’\term{Architettura} di \term{Backend}, con particolare attenzione ai componenti basati su servizi \term{Serverless} e alla loro interazione. Redazione delle sezioni della \term{Specifica Tecnica} dedicate alla descrizione delle API, dei pattern architetturali adottati e del loro utilizzo.

\item \textbf{Programmazione:} Sviluppo frontend delle principali funzionalità applicative la cui progettazione era già stata ultimata nello \term{Sprint} 8. Si fa riferimento, in particolare, alla gestione del diario, alla realizzazione della homepage e dei contatti fidati, e all’implementazione delle componenti legate al chatbot.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{RO1 (Errata stima dei tempi)}: Rischio di sottovalutare il tempo necessario per la realizzazione dei diagrammi architetturali e la scrittura delle corrispondenti sezioni all'interno del documento di \term{Specifica Tecnica}.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|C|C|C|C|C|C|}
        ...
        \hline
    \end{tabularx}
    \caption{Preventivo orario per componente - \term{Sprint} 9}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Distribuzione ore per ruolo - Preventivo \term{Sprint} 9}
\end{figure}
\paragraph{Consuntivo}
Di seguito il resoconto delle ore effettivamente impiegate. 
...
\begin{table}[H] %da aggiornare
    ...
    \begin{tabularx}{\textwidth}{|l|C|C|C|C|C|C|}
        ...
        \hline
    \end{tabularx}
    \caption{Consuntivo orario per componente - \term{Sprint} 9}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Distribuzione ore per ruolo - Consuntivo \term{Sprint} 9}
\end{figure}

\paragraph{Aggiornamento delle risorse rimanenti}
Di seguito viene riportato il prospetto delle risorse ancora disponibili dopo la chiusura dello \term{Sprint} 9. 
...
\begin{table}[H] %da aggiornare
    ...
    \begin{tabularx}{\textwidth}{|l|c|c|c|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|}
        ...
        \hline
    \end{tabularx}
    \caption{\term{Sprint} 9 - Variazione nelle risorse disponibili}
\end{table}



\paragraph{Rischi incontrati}
Durante lo \term{Sprint} non si sono verificati particolari rischi. Il lavoro è stato adeguatamente spartito in modo da limitare, per quanto possibile, rischi tecnologici come RT1. Ciò ha permesso ai membri del gruppo con meno esperienza nell'ambito della progettazione \term{Backend} di acquisire competenze e nozioni utili alla successiva fase di codifica. 

\paragraph{Retrospettiva}
La retrospettiva ha evidenziato la necessità di richiedere un maggiore supporto tecnico sia all’azienda \term{Proponente} sia al docente Cardin, al fine di valutare preventivamente la qualità e la correttezza del documento di \term{Specifica Tecnica} ed evitare revisioni sostanziali delle scelte progettuali in fase di codifica. A tal proposito, sono stati programmati due incontri all’inizio dello \term{Sprint} 10, rispettivamente con la \term{Proponente} e con il docente.

Per prevenire disallineamenti significativi tra quanto progettato e i successivi riscontri ricevuti, si è scelto di non formalizzare nel documento di \term{Specifica Tecnica} le sezioni relative all’\term{Architettura} di \term{Backend}. Questa decisione ha comportato, nel consuntivo dello \term{Sprint} 9, un numero di ore effettivamente svolte inferiore rispetto a quanto preventivato.

Pur implicando la necessità di completare le sezioni mancanti nello \term{Sprint} successivo, si ritiene che un approccio più cauto sia preferibile e consenta, nel complesso, di procedere in modo più rapido ed efficace nelle fasi successive.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|>{\columncolor{tableHeader}\bfseries}l|X|>{\columncolor{tableHeader}\bfseries}l|X|}
        ...
        \hline
    \end{tabularx}
    \caption{Riepilogo temporale \term{Sprint} 10}
\end{table}

\paragraph{Informazioni generali e attività da svolgere}
Lo \term{Sprint} 10 è orientato, ancora, al completamento del documento di \term{Specifica Tecnica}. In continuità con i progressi conseguiti nello \term{Sprint} 9, lo \term{Sprint} 10 si propone inoltre di avviare lo sviluppo lato \term{Backend} di alcune funzionalità dell’applicazione. Le attività pianificate includono:

\begin{itemize}
\item \textbf{Progettazione:} Completamento e finalizzazione dei diagrammi secondo le indicazioni ricevute durante l'incontro con il professor Cardin e con la \term{Proponente}. Completamento e formalizzazione dell’\term{Architettura} di \term{Backend}, con particolare attenzione ai componenti basati su servizi \term{Serverless} e alla loro interazione. Redazione delle sezioni della \term{Specifica Tecnica} dedicate alla descrizione delle API, dei pattern architetturali adottati e del loro utilizzo.

\item \textbf{Programmazione:} Conclusione dello sviluppo frontend delle principali funzionalità applicative la cui progettazione era già stata ultimata nello \term{Sprint} 9. Inizio dello sviluppo \term{Backend}, con particolare attenzione alla realizzazione dei servizi \term{Serverless} e alla loro integrazione con i servizi di storage e database. Si fa riferimento, in particolare, alla gestione del diario, alla realizzazione della homepage e dei contatti fidati, e all’implementazione delle componenti legate al chatbot. 
\end{itemize}
...
\begin{itemize}
    \item \textbf{RT1 (Inesperienza tecnologica)}: Data la poca familiarità con alcuni pattern architetturali e il mancato utilizzo di alcuni servizi per via di permessi non ancora del tutto concessi, la progettazione di alcune componenti del sistema potrebbe richiedere più iterazioni per raggiungere un grado di correttezza soddisfacente.
    \item \textbf{RO5 (Dipendenza da permessi esterni)}: Rischio di ritardi nello sviluppo \term{Backend} a causa della dipendenza da permessi e accessi necessari per l'utilizzo di alcuni servizi \term{Cloud}.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|C|C|C|C|C|C|}
        ...
        \hline
    \end{tabularx}
    \caption{Preventivo orario per componente - \term{Sprint} 10}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Distribuzione ore per ruolo - Preventivo \term{Sprint} 10}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|C|C|C|C|C|C|}
        ...
        \hline
    \end{tabularx}
    \caption{Consuntivo orario per componente - \term{Sprint} 10}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Distribuzione ore per ruolo - Consuntivo \term{Sprint} 10}
\end{figure}

\paragraph{Aggiornamento delle risorse rimanenti}
Di seguito viene riportato il prospetto delle risorse ancora disponibili dopo la chiusura dello \term{Sprint} 10. 
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|c|c|c|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|}
        ...
        \hline
    \end{tabularx}
    \caption{\term{Sprint} 10 - Variazione nelle risorse disponibili}
\end{table}

\paragraph{Rischi incontrati}

Durante lo \term{Sprint} 10, il rischio RT1 (Inesperienza tecnologica) si è concretizzato in parte, soprattutto per quanto riguarda la progettazione di alcune componenti del sistema.
...
Il gruppo ha evidenziato la necessità di un maggiore supporto tecnico da parte dell'azienda \term{Proponente} per quanto riguarda lo sviluppo delle funzionalità che usano AWS.
Lo sviluppo frontend è stato quasi completato, mentre lo sviluppo \term{Backend} è stato avviato lentamente, causa RO5 (Dipendenza da permessi esterni). Il gruppo continuerà a sviluppare e testare localmente, aspettando di poter eseguire test di rilascio completi su AWS non appena tutte le autorizzazioni saranno completamente concesse.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|>{\columncolor{tableHeader}\bfseries}l|X|>{\columncolor{tableHeader}\bfseries}l|X|}
        ...
        \hline
    \end{tabularx}
    \caption{Riepilogo temporale \term{Sprint} 11}
\end{table}

\paragraph{Informazioni generali e attività da svolgere}
Lo \term{Sprint} 11 è focalizzato sul proseguimento dell'implementazione delle componenti \term{Backend} e sull'aggiornamento della \term{Specifica Tecnica}. Quest'ultima attività include l'aggiunta delle sezioni relative al \term{Backend} e alle API utilizzate, con un aggiornamento delle librerie e dei pattern adottati in conformità con quanto emerso dall'incontro con il Professor Cardin avvenuto nello \term{Sprint} 10. Inoltre, è stata completata l'integrazione di AWS SAM e della relativa pipeline per automatizzare il \term{Processo} di deploy, ed è stata avviata l'integrazione dei test di \term{Backend}. Viene infine mantenuto un controllo sistematico del codice Frontend per verificare la coerenza con i diagrammi UML sviluppati dai progettisti.

Le attività pianificate includono:
\begin{itemize}
    \item \textbf{Amministrazione:} Integrazione completa di AWS SAM e configurazione della pipeline di deploy.
    \item \textbf{Progettazione:} Aggiornamento della \term{Specifica Tecnica} con l'integrazione delle parti \term{Backend}, descrizione delle API e revisione di librerie e pattern architetturali (post-incontro con il Prof. Cardin).
    \item \textbf{Programmazione:} Proseguimento dell'implementazione delle funzionalità \term{Backend}, avvio dell'integrazione dei test di \term{Backend} e revisione del codice Frontend per il rispetto dei diagrammi UML.
    \item \textbf{\term{Verifica}:} Controllo generale della qualità del codice e della sua aderenza alla progettazione tramite test e review.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|C|C|C|C|C|C|}
        ...
        \hline
    \end{tabularx}
    \caption{Preventivo orario per componente - \term{Sprint} 11}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Distribuzione ore per ruolo - Preventivo \term{Sprint} 11}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|C|C|C|C|C|C|}
        ...
        \hline
    \end{tabularx}
    \caption{Consuntivo orario per componente - \term{Sprint} 11}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Distribuzione ore per ruolo - Consuntivo \term{Sprint} 11}
\end{figure}

\paragraph{Aggiornamento delle risorse rimanenti}
Di seguito viene riportato il prospetto delle risorse ancora disponibili dopo la chiusura dello \term{Sprint} 11.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|c|c|c|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|}
        ...
        \hline
    \end{tabularx}
    \caption{\term{Sprint} 11 - Variazione nelle risorse disponibili}
\end{table}

\paragraph{Rischi incontrati}
Durante lo \term{Sprint} 11 si sono manifestati i seguenti rischi:
\begin{itemize}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|>{\columncolor{tableHeader}\bfseries}l|X|>{\columncolor{tableHeader}\bfseries}l|X|}
        ...
        \hline
    \end{tabularx}
    \caption{Riepilogo temporale \term{Sprint} 12}
\end{table}

\paragraph{Informazioni generali e attività da svolgere}
Lo \term{Sprint} 12 si focalizza sul completamento della parte di \term{Backend} del progetto e sull'integrazione con il Frontend delle funzionalità implementate. In seguito al completamento della parte di codice si andrà ad aggiornare il documento di \term{Specifica Tecnica} con informazioni più recenti e in linea con lo sviluppo. Infine si valuteranno i requisiti funzionali insieme all'azienda \term{Proponente}, eventualmente riclassificandoli se necessario in vista della consegna del \term{MVP}.\\ 
Le attività pianificate includono:
\begin{itemize}
    \item \textbf{Progettazione:} Aggiornamento della \term{Specifica Tecnica}, revisione della sezione dell'\term{Architettura} logica e correzione dei diagrammi delle classi. Completamento sezioni mancanti.
    \item \textbf{Programmazione:} Completare parti mancanti del \term{Backend}, effettuare il deploy della funzionalità chatbot e integrare il Frontend con il \term{Backend} delle funzionalità completate.
    \item \textbf{\term{Verifica}:} Effettuare controlli sulla qualità del codice \term{Backend} e sulla correttezza delle modifiche alla documentazione.
\end{itemize}

\paragraph{Rischi attesi}
\begin{itemize}
    \item \textbf{RT1 (Inesperienza tecnologica)}: Possibili difficoltà nello sviluppo del \term{Backend} dell'applicazione dovuta a inesperienza con i sistemi AWS, in particolare AWS SAM.
    \item \textbf{RO1 (Errata stima dei tempi)}: Rischio di sottovalutare la complessità dell'integrazione tra Frontend e \term{Backend}.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|C|C|C|C|C|C|}
        ...
        \hline
    \end{tabularx}
    \caption{Preventivo orario per componente - \term{Sprint} 12}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Distribuzione ore per ruolo - Preventivo \term{Sprint} 12}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|C|C|C|C|C|C|}
        ...
        \hline
    \end{tabularx}
    \caption{Consuntivo orario per componente - \term{Sprint} 12}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Distribuzione ore per ruolo - Consuntivo \term{Sprint} 12}
\end{figure}

\paragraph{Aggiornamento delle risorse rimanenti}
Di seguito viene riportato il prospetto delle risorse ancora disponibili dopo la chiusura dello \term{Sprint} 12.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|c|c|c|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|}
        ...
        \hline
    \end{tabularx}
    \caption{\term{Sprint} 12 - Variazione nelle risorse disponibili}
\end{table}

\paragraph{Rischi incontrati}
Durante lo \term{Sprint} 12 si sono manifestati i seguenti rischi:
\begin{itemize}
    \item \textbf{RT1 (Inesperienza tecnologica):} La poca familiarità con AWS SAM ha causato dei problemi nel completamento del \term{Backend}, rendendo necessarie più ore lavorative.
\end{itemize}

\paragraph{Retrospettiva} Lo \term{Sprint} 12 ha permesso al gruppo di portare molto vicino al completamento l'applicazione, nonostante le difficoltà riscontrate con gli strumenti per il \term{Backend}. Si è resa nota la necessità di aggiornare nuovamente i contenuti della \term{Specifica Tecnica} a seguito delle attività di sviluppo, perciò sarà necessaria una quantità limitata di lavoro aggiuntivo, anche per terminare le funzionalità non ancora in linea con i requisiti dell'applicazione. \\
