...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|L|Z|}
        ...
        \hline
        \rowcolor{tableLineOdd}
        Redazione \term{Specifica Tecnica} & Documento che descrive l'architettura del sistema, i diagrammi delle classi e le scelte progettuali effettuate per le componenti \term{Backend} e \term{Frontend}. Spiega inoltre come le varie classi sono collegate e comunicano tra loro. \\
        \hline
        \rowcolor{tableLineEven}
        ...
        \hline
        \rowcolor{tableLineEven}
        Completamento \term{Norme di progetto} & Integrazione delle norme e delle convenzioni di codifica specifiche per i linguaggi Python e Dart, garantendo la produzione di codice corretto, condivisibile e comprensibile. \\
        \hline
        \rowcolor{tableLineOdd}
        ...
        \hline
        \rowcolor{tableLineEven}
        Ampliamento \term{Glossario} & Inserimento dei nuovi termini tecnici e degli acronimi emersi durante la fase di sviluppo della \term{Product Baseline} per garantire chiarezza terminologica. \\
        \hline
        \rowcolor{tableLineOdd}
        Realizzazione del Minimum Viable Product (MVP) & Sviluppo effettivo del codice per le componenti \term{Frontend} e \term{Backend}, comprensivo della realizzazione dei relativi test per assicurarne il corretto funzionamento. \\
        \hline
    \end{tabularx}
...
\paragraph{Informazioni generali e attività da svolgere}
Lo \term{Sprint} 11 è focalizzato sul proseguimento dell'implementazione delle componenti \term{Backend} e sull'aggiornamento della \term{Specifica Tecnica}. Quest'ultima attività include l'aggiunta delle sezioni relative al \term{Backend} e alle API utilizzate, con un aggiornamento delle librerie e dei pattern adottati in conformità con quanto emerso dall'incontro con il Professor Cardin avvenuto nello \term{Sprint} 10. Inoltre, è stata completata l'integrazione di AWS SAM e della relativa pipeline per automatizzare il processo di deploy, ed è stata avviata l'integrazione dei test di backend. Viene infine mantenuto un controllo sistematico del codice \term{Frontend} per verificare la coerenza con i diagrammi UML sviluppati dai progettisti.
...
\begin{itemize}
    ...
    \item \textbf{Programmazione:} Proseguimento dell'implementazione delle funzionalità \term{Backend}, avvio dell'integrazione dei test di backend e revisione del codice \term{Frontend} per il rispetto dei diagrammi UML.
...
\begin{itemize}
    ...
    \item \textbf{RO1 (Errata stima dei tempi)}: Rischio di sottovalutare l'impegno richiesto per il controllo di coerenza tra il codice \term{Frontend} e i diagrammi UML.
\end{itemize}
