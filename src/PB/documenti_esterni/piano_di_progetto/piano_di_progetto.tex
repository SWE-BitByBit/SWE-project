...
Ecco il codice LaTeX aggiornato con i nuovi calcoli (tabelle, delta (+/-), budget rimanente e grafici a torta sistemati con le nuove angolazioni) e i testi revisionati per riflettere questo ottimo andamento. Puoi sostituire tutto il blocco dello \term{Sprint} 8 con questo:
Snippet di codice
...
\paragraph{Informazioni generali e attività da svolgere}
Lo \term{Sprint} 8 segna l'inizio dei lavori per la Product \term{Baseline} (\term{PB}), spostando il focus sull'impostazione architetturale dell'\term{MVP}, la ripianificazione a lungo termine e l'aggiornamento della documentazione a seguito dei feedback ricevuti dai docenti (Prof. Vardanega e Prof. Cardin).
Le attività pianificate e svolte includono:
\begin{itemize}
    \item \textbf{Gestione (Responsabile):} Creazione delle macro-\term{Issue} nel \term{Project} GitHub e conseguente aggiornamento del diagramma di Gantt. Definizione della nuova \term{Milestone} per la \term{PB} con i relativi artefatti concordati dal gruppo. Miglioramento del file Excel per la rendicontazione delle ore. Redazione dei verbali degli incontri con la \term{Proponente} Miriade (12 e 30 marzo), resisi necessari per incrementare la comunicazione come suggerito dal docente, e relativa creazione di \term{Issue}. Correzione di alcuni appunti documentali (es. rimozione numerazione sezioni dall'indice del Glossario e aggiornamento date nel PdP).
    \item \textbf{Amministrazione:} Setup della \term{Repository} per l'\term{MVP} (progetto Flutter, directory sorgente/test, progetto AWS SAM). Configurazione delle pipeline di \term{CI/CD} tramite \term{GitHub Actions} per il deploy su AWS e integrazione di SonarQube \term{Cloud} per l'\term{Analisi Statica} di Python e Dart (verificando il supporto per quest'ultimo). Aggiornamento corposo delle \textit{Norme di Progetto} con convenzioni di scrittura codice, linee guida architetturali per Flutter e documentazione del \term{Processo} di sviluppo sulla wiki. Correzioni strutturali sui processi (gerarchia \term{Processo}-attività-procedure-strumenti) e sulla veste del sito web (es. riposizionamento della Lettera di Presentazione, rimozione etichette "File pdf").
    \item \textbf{Analisi:} Revisione e ampliamento dell'\textit{\term{Analisi dei requisiti}} in risposta ai feedback ricevuti. Modifica dei diagrammi UML, aggiornamento del tracciamento requisiti e in particolare l'ampliamento dell'UC48 (Consultazione chatbot), precedentemente giudicato troppo generico.
    \item \textbf{Progettazione:} Creazione del documento \textit{\term{Specifica Tecnica}} (scopo, tecnologie impiegate). Inizio della progettazione dell'\term{Architettura} logica e di \term{Deployment}, con focus sulle funzionalità primarie (chatbot, diario criptato/fittizio, contatti fidati). Realizzazione di bozze su Figma per l'aspetto dell'app. Creazione di \textit{Makefile} per l'esecuzione agevolata dei test.
\end{itemize}
