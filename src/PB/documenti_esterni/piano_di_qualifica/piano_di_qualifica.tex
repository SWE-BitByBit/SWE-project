...
\end{tabularx}
\caption{Soglie delle metriche per il \protect\term{Processo} di fornitura}
\end{table}
...
\end{tabularx}
\caption{Soglie delle metriche per il \protect\term{Processo} di documentazione}
\end{table}
...
\hline
\rowcolor{white}
\caption[]{Tabella dei \protect\term{Test di Sistema}} \\
\endfoot
\hline
\caption{Tabella dei \protect\term{Test di Sistema}} \\
\endlastfoot
...
\hline
\endfirsthead
\caption[]{Tracciamento dei \protect\term{Test di Sistema}} \\
\hline
\rowcolor{gray!60}
\textbf{Codice Test} & \textbf{Codice \term{Requisito}} \\
\hline
...
\hline
\rowcolor{white}
\caption{Tracciamento dei \protect\term{Test di Sistema}} \label{tab:tracciamento_test_sistema} \\
\end{longtable}
\endgroup
...
\hline
\rowcolor{white}
\caption[]{Tabella dei \protect\term{Test di Accettazione}} \\
\endfoot
\hline
\rowcolor{white}
\caption{Tabella dei \protect\term{Test di Accettazione}} \\
...
\hline
\rowcolor{white}
\caption[]{Tabella dei \protect\term{Test di Unità} - Frontend} \\
\endfoot
\hline
\caption{Tabella dei \protect\term{Test di Unità} - Frontend} \\
\endlastfoot
...
\hline
\rowcolor{white}
\caption[]{Tabella dei \protect\term{Test di Unità} - \protect\term{Backend}} \\
\endfoot
\hline
\caption{Tabella dei \protect\term{Test di Unità} - \protect\term{Backend}} \\
\endlastfoot
...
\hline
TB-142-U & [DMSController] Verificare che l'evento schedulato decrementi correttamente il contatore per il primo \term{Utente}. & I \\
\hline
TB-143-U & [DMSController] Verificare che l'evento schedulato decrementi correttamente il contatore per il terzo \term{Utente}. & I \\
\hline
TB-144-U & [DMSController] Verificare che l'evento schedulato decrementi correttamente il contatore per il quarto \term{Utente}. & I \\
\hline
TB-145-U & [DMSController] Verificare che l'evento schedulato invii l'email di allerta all'\term{Utente}. & I \\
\hline
...
\begin{longtable}{|c|p{10cm}|c|}
    ...
    TF-1-I & \term{Verifica} dell'integrazione end-to-end: navigazione verso la schermata "Luoghi Sicuri" a partire dalla dashboard principale. & I \\
    \hline
    TF-2-I & \term{Verifica} dell'integrazione end-to-end: corretto caricamento e rendering della mappa interattiva. & I \\
    \hline
    TF-3-I & \term{Verifica} dell'integrazione end-to-end: apertura e visualizzazione della legenda informativa della mappa. & I \\
    \hline
    TF-4-I & \term{Verifica} dell'integrazione end-to-end: interazione con il pulsante di geolocalizzazione. & I \\
    \hline
    TF-5-I & \term{Verifica} dell'integrazione end-to-end: rendering dei marker sulla mappa. & I \\
    \hline

    % --- Integrazione Frontend: Materiale Informativo ---
    TF-6-I & \term{Verifica} dell'integrazione end-to-end: navigazione verso la schermata "Materiale Informativo" a partire dalla dashboard principale. & I \\
    \hline
    TF-7-I & \term{Verifica} dell'integrazione end-to-end: rendering della lista dei materiali informativi. & I \\
    \hline
    TF-8-I & \term{Verifica} dell'integrazione end-to-end: funzionamento dei filtri per categoria e conseguente aggiornamento reattivo della vista. & I \\
    \hline
    TF-9-I & \term{Verifica} dell'integrazione end-to-end: ripristino della lista completa deselezionando i filtri. & I \\
    \hline
    ...
    \hline
    TF-12-I & [Chatbot] Verificare l'invio di un messaggio testuale dall'\term{Utente}. & I \\
    \hline
    ...
    TF-15-I & \term{Verifica} dell'integrazione end-to-end: navigazione verso la schermata "Diario" a partire dalla dashboard. & I \\
    \hline
    TF-16-I & \term{Verifica} dell'integrazione end-to-end: inserimento della password e sblocco del diario. & I \\
    \hline
    TF-17-I & \term{Verifica} dell'integrazione end-to-end: caricamento e visualizzazione della lista delle note. & I \\
    \hline
        % --- Integrazione Frontend: Contatti Fidati ---
    TF-18-I & \term{Verifica} dell'integrazione end-to-end: navigazione verso la schermata "Contatti Fidati" a partire dalla dashboard. & I \\
    \hline
    TF-19-I & \term{Verifica} dell'integrazione end-to-end: caricamento e visualizzazione della lista dei contatti. & I \\
    \hline
    TF-20-I & \term{Verifica} dell'integrazione end-to-end: compilazione del form e aggiunta di un nuovo contatto fidato. & I \\
    \hline
...
\begin{longtable}{|l|p{10cm}|c|}
    ...
    \hline
    \caption{Tracciamento dei \protect\term{Test di Integrazione} \protect\term{Backend} per materiali informativi e luoghi sicuri}
    \endlastfoot

    TB-1-I & \term{Verifica} dell'integrazione tra il componente \texttt{S3ResourcesRepository} e il servizio Amazon S3 per il recupero e la mappatura dei materiali informativi. & I \\
    \hline
    TB-2-I & \term{Verifica} dell'integrazione tra l'entry point Lambda dei materiali informativi e la catena di componenti interni (Controller, Service, \ignoreglossary{Repository}). & I \\
    \hline
    TB-3-I & \term{Verifica} dell'integrazione tra il componente \texttt{S3SafePlacesRepository} e il servizio Amazon S3 per il recupero dei marker relativi ai luoghi sicuri. & I \\
    \hline
    TB-4-I & \term{Verifica} dell'integrazione tra l'entry point Lambda dei luoghi sicuri e la catena di componenti interni (Controller, Service, \ignoreglossary{Repository}). & I \\
    \hline
    TB-5-I & \term{Verifica} dell'integrazione tra il componente \texttt{DmsRepository} e il servizio Amazon DynamoDB per la persistenza e gestione delle configurazioni Dead Man's Switch. & I \\
    \hline
    TB-6-I & \term{Verifica} dell'integrazione tra il componente \texttt{TrustedContactRepository} e il servizio Amazon DynamoDB per la gestione dei contatti di emergenza. & I \\
    \hline
    TB-7-I & \term{Verifica} dell'integrazione tra il componente \texttt{NotificationRepository} e il servizio Amazon SES per l'invio delle email di notifica e di allerta. & I \\
    \hline
    TB-8-I & \term{Verifica} dell'integrazione dell'entry point Lambda dei contatti fidati con l'intera catena di componenti interni (Controller, Services, Repositories). & I \\
    \hline
    TB-9-I & \term{Verifica} dell'integrazione tra il servizio ChatbotCRUDService e il servizio Amazon DynamoDB per la persistenza delle conversazioni e della cronologia dei messaggi. & I \\
    \hline
    TB-10-I & \term{Verifica} dell'integrazione dell'entry point Lambda del chatbot con l'intera catena di componenti interni (Controller, Service, \ignoreglossary{Repository}) e il mock del servizio LLM. & I \\
    \hline
    TB-11-I & \term{Verifica} dell'integrazione dell'intero flusso Lambda per l'attivazione manuale dell'allarme SOS e il corretto innesco del \texttt{NotificationRepository} verso Amazon SES. & I \\
    \hline
    TB-12-I & \term{Verifica} dell'integrazione del flusso scatenato dal cron job (EventBridge) per il ricalcolo automatico dei timer del Dead Man's Switch e il conseguente invio di allerte. & I \\
    \hline
    TB-13-I & \term{Verifica} dell'integrazione del flusso API per l'operazione di heartbeat, assicurando il corretto azzeramento dei contatori del Dead Man's Switch su DynamoDB. & I \\
    \hline
    TB-14-I & \term{Verifica} dell'integrazione tra il componente \texttt{DynamoAuthAdapter} e il servizio Amazon DynamoDB per la persistenza delle credenziali di accesso (reali e fake) al diario. & I \\
    \hline
    TB-15-I & \term{Verifica} dell'integrazione tra l'entry point Lambda dell'autenticazione diario e l'intera catena di componenti interni (Controller, Service, \ignoreglossary{Repository}). & I \\
    \hline
    TB-16-I & \term{Verifica} dell'integrazione tra il componente \texttt{DynamoNoteAdapter} e il servizio Amazon DynamoDB per la persistenza delle note e dei relativi elementi multimediali. & I \\
    \hline
    TB-17-I & \term{Verifica} dell'integrazione tra il componente \texttt{S3NoteAdapter} e il servizio Amazon S3 per la gestione del caricamento e dell'eliminazione degli allegati alle note. & I \\
    \hline
    TB-18-I & \term{Verifica} dell'integrazione dell'entry point Lambda della gestione note con l'intera catena di componenti interni (Controller, Service, Repositories) per le operazioni CRUD. & I \\
    \hline 
    TB-19-I & \term{Verifica} dell'integrazione del flusso di eliminazione note, assicurando la corretta rimozione sincronizzata dei dati su DynamoDB e dei file correlati su Amazon S3. & I \\
    \hline
\end{longtable}
