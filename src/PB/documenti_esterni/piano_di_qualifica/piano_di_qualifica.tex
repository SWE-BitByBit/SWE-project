...
\stepcounter{testsys} \systestcode & Verificare che il pulsante SOS rapido sia visibile e facilmente accessibile nell'interfaccia. & RF-OB-99 & I \\
\hline
...
\stepcounter{testsys} \systestcode & Verificare che l'accesso alla sezione Chatbot con l'AI avvenga correttamente. & RF-OB-103 & I \\
\hline
...
\hline
TF-2-U & [MaterialViewModel] Verificare che, in assenza di un filtro attivo, vengano restituiti tutti i materiali presenti nel repository. & I \\
\hline
...
\hline
TF-13-U & [SafePlaceViewModel] Verificare che il metodo \texttt{loadPlaces} aggiorni correttamente lo stato interno tramite il repository. & I \\
\hline
...
\hline
TF-19-U & [SafePlaceViewModel] Verificare che il metodo \texttt{saveMapSessionState} aggiorni la cache del repository senza generare notifiche alla UI. & I \\
\hline
...
\hline
TF-25-U & [AuthViewModel] Verificare che il comando \texttt{login} gestisca correttamente un errore restituito dal repository. & I \\
\hline
TF-26-U & [AuthViewModel] Verificare che il comando \texttt{login} sollevi un'eccezione qualora il repository restituisca un valore \texttt{null}. & I \\
\hline
TF-27-U & [AuthViewModel] Verificare che il comando \texttt{login} sollevi un'eccezione qualora il repository lanci un errore imprevisto. & I \\
\hline
TF-28-U & [AuthViewModel] Verificare che il comando \texttt{logout} invochi correttamente il repository e pulisca la cache locale. & I \\
\hline
TF-29-U & [AuthViewModel] Verificare che il comando \texttt{logout} sollevi un'eccezione qualora il repository di disconnessione fallisca. & I \\
\hline
...
\hline
TF-32-U & [HomeViewModel] Verificare che la proprietà \texttt{isDeadManActive} restituisca \texttt{true} quando il repository contiene impostazioni attive. & I \\
\hline
TF-33-U & [HomeViewModel] Verificare che la proprietà \texttt{isDeadManActive} restituisca \texttt{false} quando il repository contiene impostazioni disattivate. & I \\
\hline
TF-34-U & [HomeViewModel] Verificare che la proprietà \texttt{isDeadManActive} restituisca \texttt{false} quando il repository non contiene impostazioni. & I \\
\hline
...
TF-45-U & [TrustedContactViewModel] Verificare che il metodo \texttt{createContact} invochi il repository e aggiorni la lista dei contatti. & I \\
\hline
TF-46-U & [TrustedContactViewModel] Verificare che il metodo \texttt{updateContact} invochi il repository e aggiorni la lista dei contatti. & I \\
\hline
...
\hline
TF-48-U & [TrustedContactViewModel] Verificare che il metodo \texttt{deleteContact} esegua il rollback e aggiorni la lista in caso di fallimento del repository. & I \\
\hline
TF-49-U & [TrustedContactViewModel] Verificare che il metodo \texttt{loadContacts} imposti correttamente lo stato di errore in caso di fallimento del repository. & I \\
\hline

% --- Dead Man's Switch: DeadManViewModel ---

TF-50-U & [DeadManViewModel] Verificare che, al caricamento, la bozza delle impostazioni venga popolata con una copia dei dati presenti nel repository. & I \\
\hline
...
\hline
TF-54-U & [DeadManViewModel] Verificare che il metodo \texttt{discardChanges} ripristini la bozza con i valori attualmente salvati nel repository. & I \\
\hline
TF-55-U & [DeadManViewModel] Verificare che il metodo \texttt{saveSettings} invochi il repository e aggiorni la bozza con i nuovi dati salvati. & I \\
\hline
...
\hline
TF-66-U & [DiaryAccessViewModel] Verificare che \texttt{updateRealPassword} aggiorni lo stato correttamente in caso di successo del repository. & I \\
\hline
...
\hline
TF-71-U & [DiaryViewModel] Verificare che il metodo \texttt{loadNotes} invochi il metodo \texttt{getNotes} sul repository. & I \\
\hline
...
\hline
TF-73-U & [DiaryViewModel] Verificare che il metodo \texttt{saveNote} invochi il metodo \texttt{saveNote} sul repository. & I \\
\hline
...
TF-101-U & [CacheManager] Verificare che il metodo \texttt{clearAllCaches} invochi \texttt{clearCache} su ciascun repository registrato. & I \\
\hline
TF-102-U & [CacheManager] Verificare che il metodo \texttt{clearAllCaches} non esegua alcuna operazione quando la lista dei repository è vuota. & I \\
\hline
...
\hline
\rowcolor{white}
\caption[]{Tabella dei \term{Test di Unità} - Backend} \\
\endfoot
\hline
\caption{Tabella dei \term{Test di Unità} - Backend} \\
\endlastfoot
...
TB-56-U & [SafePlacesService] Verificare che il metodo \texttt{get\_all\_markers} deleghi correttamente l'operazione al repository. & I \\
\hline
TB-57-U & [SafePlacesService] Verificare che il metodo \texttt{get\_all\_markers} propaghi correttamente l'eccezione sollevata dal repository. & I \\
\hline
...
\hline
TB-62-U & [LambdaHandlerSafePlaces] Verificare che il Lambda handler gestisca correttamente il payload API Gateway v2. & I \\
\hline
...
TB-95-U & [ChatbotLambdaHandler] Verificare che le richieste prive di identificativo utente vengano rifiutate come non autorizzate. & I \\
\hline
...
\hline
TB-142-U & [Controller] Verificare che l'evento schedulato decrementi correttamente il contatore per il primo utente. & I \\
\hline
TB-143-U & [Controller] Verificare che l'evento schedulato decrementi correttamente il contatore per il terzo utente. & I \\
\hline
TB-144-U & [Controller] Verificare che l'evento schedulato decrementi correttamente il contatore per il quarto utente. & I \\
\hline
TB-145-U & [Controller] Verificare che l'evento schedulato invii l'email di allerta all'utente. & I \\
\hline
