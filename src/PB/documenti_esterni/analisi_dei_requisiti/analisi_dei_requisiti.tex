...
\hline
\rowcolor{white}
2.0.0 & 2026-05-06 & Giovanni Visentin & Ultime modifiche e approvazione documento & Dennis Parolin \\
\hline
\rowcolor{gray!20}
1.7.0 & 2026-05-01 & Giovanni Visentin & Modifiche generali e aggiornamenti dei requisiti & Dennis Parolin \\
\hline
\rowcolor{white}
1.6.1 & 2026-03-29 & Giovanni Visentin & Riscrittura del sistema di figure contententi errori & Dennis Parolin \\
\hline
\rowcolor{gray!20}
1.6.0 & 2026-03-20 & Dennis Parolin & Riscrittura della sezione \hyperref[subsection:requisiti_funzionali_obbligatori]{Requisiti Funzionali Obbligatori}, aggiunte le sezioni \hyperref[subsection:requisiti_funzionali_desiderabili]{Requisiti Funzionali Desiderabili} e \hyperref[subsection:requisiti_funzionali_opzionali]{Requisiti Funzionali Opzionali}. & Gabriele Scaggiante \\
\hline
\rowcolor{white}
...
\hline
\rowcolor{gray!20}
0.20.0 & 2026-02-09 & Dennis Parolin & Riscritta la sezione "Requisiti" (\ref{section:requisiti}) con le sue sottosezioni (\ref{subsection:requisiti_funzionali_obbligatori}, \ref{subsection:requisiti_qualita}, \ref{subsection:requisiti_vincolo}) sulla base dei nuovi casi d'uso redatti. & Giovanni Visentin \\
\hline
\rowcolor{white}
...
\hline
\rowcolor{white}
0.14.0 & 2025-12-22 & Giovanni Visentin & Modifica UC-IMP8[deprecato] in IMP9[deprecato] e aggiunti UC-I[deprecato] & Dennis Parolin \\
\hline
\rowcolor{gray!20}
0.13.0 & 2025-12-21 & Giovanni Visentin & Aggiunto caso d'uso UC-IMP8[deprecato] e proposto UC-SYS1[deprecato] & Dennis Parolin \\
\hline
\rowcolor{white}
0.12.0 & 2025-12-19 & Riccardo Manisi & Aggiunta UC-D1[deprecato], UC-D2[deprecato] per impostazione e modifica password diario criptato. Eliminato UC-BW1.6 [deprecato] & Dennis Parolin \\
\hline
\rowcolor{gray!20}
...
\hline
\rowcolor{white}
0.3.0 & 2025-12-11 & Dennis Parolin & Redazione della sezione \ref{sec:funzioni_prodotto}. Modifica sezione \ref{sec:prospettiva_prodotto} sulla base delle funzionalità concordate con la Proponente. Spostato il registro delle modifiche a prima dell'indice. & Giovanni Visentin \\
\hline
\rowcolor{gray!20}
...
Il presente documento di Analisi dei requisiti ha l'obiettivo di esporre in modo esaustivo e non ambiguo le specifiche funzionali e vincolanti del sistema software oggetto di sviluppo.
...
La prima ha l'obiettivo di analizzare comportamenti o narrazioni dell'Utente per identificare la situazione attuale e riconoscere pattern di pericolo meritevoli di attenzione. La seconda è orientata all'autoconsapevolezza, offrendo uno spazio privato per l'analisi dei propri pensieri. L'Utente potrà cambiarere il contesto di conversazione tra le due modalità, mantenendo la possibilità di accedere a entrambe in qualsiasi momento. In questa fase, l'IA non avrà alcun ruolo attivo o decisionale in situazioni critiche, ma si limiterà a fornire supporto analitico e riflessivo.
...
\subsubsection{Dead Man's Switch}
Il sistema prevede un meccanismo di controllo denominato "Dead Man's Switch", che si attiva in caso di prolungata inattività dell'Utente. Se l'Utente non interagisce con l'app per un periodo di tempo configurabile, il sistema invierà una notifica all'Utente per controllare la propria situazione. In caso di mancata risposta, il sistema, dopo un ultieriore periodo di tempo, invierà una notifica ai contatti fidati, segnalando la potenziale situazione di rischio. Una qualsiasi interazione dell'Utente con l'app durante questo Processo annullerà le notifiche e resetterà il timer di controllo.
L'Utente avrà la possibilità di disattivare questa funzionalità in qualsiasi momento tramite le impostazioni.

\subsubsection{Luoghi Sicuri}
La sezione "Luoghi Sicuri" fornirà una mappa interattiva che evidenzia i luoghi considerati sicuri, come centri antiviolenza, stazioni di polizia, ospedali e altri punti di riferimento. L'Utente potrà visualizzareil luogo sicuro più vicino a sé. Questa funzionalità sarà integrata con Google Maps per garantire accuratezza e facilità d'uso.
...
\endhead
\rowcolor{white}
\centering \textbf{Utente Generico} & Un Utente che non ha eseguito l'autenticazione all'applicazione e che non ha ancora creato una password per il diario criptato. Il sistema permette a questo attore di eseguire un sottoinsieme della totalità delle funzionalità. \\
\hline
\rowcolor{gray!20}
\centering \textbf{Utente Autenticato} & Un Utente il quale ha eseguito l'autenticazione all'applicazione. Il sistema possiede l'indirizzo mail di questo attore. Ha accesso a delle funzionalità aggiuntive rispetto all'Utente Generico e all'Utente autenticato al diario. \\
\hline
\rowcolor{white}
\centering \textbf{Utente autenticato al diario} & Utente che ha impostato la password per il diario criptato. \\
\hline
\rowcolor{gray!70}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-1-auth.pdf}
...
\subsubsection{UC-14 Logout} \label{subsec:UC-14}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-14-logout.pdf}
    \caption{Diagramma del caso d'uso UC-14}
    \label{fig:UC-14}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{Post-condizioni:}
    \begin{itemize}
        \item L’attore non è più autenticato all'applicazione
    \end{itemize}
    \item \textbf{Scenario principale:}
    \begin{itemize}
        \item L'attore interagisce con la voce per effettuare il logout dall'applicazione
        \item Il sistema termina la sessione dell'attore
    \end{itemize}
    \item \textbf{Trigger:} L'attore vuole effettuare il logout
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-3-switchmode.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-4-lista-contatti-fidati.pdf}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item L'attore può ricaricare la lista dei contatti fidati
    \end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-6-visualizzazione-info-contatti-fidati.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-7-aggiunta-contatto-fidato.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-12-modifica-contatto-fidato.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-13-impostazioni.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-15-16-dmswitch.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-17-dmswitch.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-18-dmswitch.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-19-20-dmswitch.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-21-impostazione-password-diario-criptato.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-23-impostazione-password-diario-fittizio.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-25-inserimento-password-accesso-diario.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-28-visualizzazione-diario-criptato.pdf}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item L'attore può ricaricare la lista delle note del diario criptato
    \end{itemize}
    \item \textbf{Inclusioni:}
    \begin{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-30-diario.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-31-diario.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-32-diario.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-33-34-diario.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-35-36-diario.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-29-visualizzazione-diario-fittizio.pdf}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Il sistema non è in grado di recuperare le note dal database $\rightarrow$ \hyperref[sec:UC-5]{UC-5}
        \item L'attore può ricaricare la lista delle note del diario fittizio
    \end{itemize}
    \item \textbf{Inclusioni:}
    \begin{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-37-diario_fittizio.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-38-diario_fittizio.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-39-diario_fittizio.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-40-41-diario_fittizio.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-42-43-diario_fittizio.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-44-47-guardiano.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-48-detective.pdf}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Il chatbot analizza il messaggio secondo il contenuto del diario criptato
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-51-detective.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-52-detective.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-54-55-detective.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-56-detective.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-57-detective.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-58-coraggio.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-59-coraggio.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-60-coraggio.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-61-coraggio.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-62-coraggio.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-63-coraggio.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-64-coraggio.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-65-coraggio.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{use_case/UC-66-67-coraggio.pdf}
...
\hline
\endfirsthead
\caption[]{Tabella Requisiti Funzionali obbligatori (continua)} \\
\hline
\rowcolor{gray!60}
\textbf{Codice} & \textbf{Descrizione} & \textbf{Riferimento} \\
\hline
...
RF-OB-1 & L'Utente deve poter autenticarsi all'applicazione tramite l'utilizzo di Google Auth & \hyperref[subsec:UC-1]{UC-1} \\
\hline

RF-OB-2 & Il sistema deve attivare automaticamente la funzionalità di Dead Man's Switch a seguito del login dell'Utente & \hyperref[subsec:UC-1]{UC-1}, \hyperref[sec:UC-19]{UC-19} \\
\hline

RF-OB-3 & L'Utente deve poter visualizzare un messaggio di errore esplicativo nel caso in cui l'autenticazione tramite Google Auth fallisca, venga annullata o restituisca parametri non validi & \hyperref[subsec:UC-2]{UC-2} \\
\hline

RF-OB-5 & L'Utente deve poter visualizzare la lista dei contatti fidati & \hyperref[sec:UC-4]{UC-4} \\
\hline

RF-OB-6 & L'Utente deve poter visualizzare un messaggio di errore esplicativo nel caso in cui il recupero dei dati dal database fallisca & \hyperref[sec:UC-5]{UC-5} \\
\hline

RF-OB-7 & L'Utente deve poter ricaricare la sezione dei contatti fidati dopo la visualizzazione dell'errore di caricamento dei dati dal database & \hyperref[sec:UC-4]{UC-4}, \hyperref[sec:UC-5]{UC-5} \\
\hline
...
RF-OB-9 & L'Utente deve poter visualizzare la preview delle informazioni (il nome) del contatto fidato che sta visualizzando dalla lista dei contatti fidati & \hyperref[subsec:UC-4.1.1]{UC-4.1.1} \\
\hline

RF-OB-10 & L'Utente deve poter visualizzare i dettagli di un contatto fidato selezionato dalla lista dei contatti fidati & \hyperref[subsec:UC-6]{UC-6} \\
\hline

RF-OB-11 & L'Utente deve poter visualizzare il nome del contatto fidato selezionato dalla lista dei contatti fidati & \hyperref[subsec:UC-6.1]{UC-6.1} \\
\hline

RF-OB-12 & L'Utente deve poter visualizzare il numero di telefono del contatto fidato selezionato dalla lista dei contatti fidati & \hyperref[subsec:UC-6.2]{UC-6.2} \\
\hline

RF-OB-13 & L'Utente deve poter visualizzare l'indirizzo email del contatto fidato selezionato dalla lista dei contatti fidati & \hyperref[subsec:UC-6.3]{UC-6.3} \\
\hline

RF-OB-14 & L'Utente deve poter aggiungere un nuovo contatto fidato alla lista dei contatti fidati & \hyperref[subsec:UC-7]{UC-7} \\
\hline

RF-OB-15 & L'Utente deve poter inserire il nome del nuovo contatto fidato durante l'aggiunta di esso alla lista dei contatti fidati & \hyperref[subsec:UC-7.1]{UC-7.1} \\
\hline

RF-OB-16 & L'Utente deve poter inserire il numero di telefono del nuovo contatto fidato durante l'aggiunta di esso alla lista dei contatti fidati & \hyperref[subsec:UC-7.2]{UC-7.2} \\
\hline

RF-OB-17 & L'Utente deve poter inserire l'indirizzo email del nuovo contatto fidato durante l'aggiunta di esso alla lista dei contatti fidati & \hyperref[subsec:UC-7.3]{UC-7.3} \\
\hline
...
RF-OB-19 & L'Utente deve poter visualizzare un messaggio di errore esplicativo nel caso in cui le informazioni di numero di telefono e email siano già presenti nel sistema & \hyperref[subsec:UC-9]{UC-9}, \hyperref[subsec:UC-10]{UC-10} \\
\hline

RF-OB-20 & L'Utente deve poter reinserire le informazioni di numero di telefono e email dopo la visualizzazione del messaggio di errore esplicativo & \hyperref[subsec:UC-8]{UC-8}, \hyperref[subsec:UC-9]{UC-9}, \hyperref[subsec:UC-10]{UC-10}, \hyperref[subsec:UC-11]{UC-11} \\
\hline
...
RF-OB-25 & L'Utente deve poter modificare i dati di un contatto fidato esistente & \hyperref[subsec:UC-12]{UC-12} \\
\hline

RF-OB-26 & L'Utente deve poter modificare il nome di un contatto fidato esistente & \hyperref[subsec:UC-12.1]{UC-12.1} \\
\hline

RF-OB-27 & L'Utente deve poter modificare il numero di telefono di un contatto fidato esistente & \hyperref[subsec:UC-12.2]{UC-12.2} \\
\hline

RF-OB-28 & L'Utente deve poter modificare l'indirizzo email di un contatto fidato esistente & \hyperref[subsec:UC-12.3]{UC-12.3} \\
\hline

RF-OB-29 & L'Utente deve poter eliminare un contatto fidato & \hyperref[subsec:UC-13]{UC-13} \\
\hline
...
RF-OB-31 & L'Utente deve poter confermare, con un'apposita voce, l'eliminazione di un contatto fidato quando il sistema richiede la conferma & \hyperref[subsec:UC-13.1]{UC-13.1} \\
\hline

RF-OB-32 & L'Utente deve poter effettuare il logout dopo essersi autenticato all'applicazione & \hyperref[subsec:UC-14]{UC-14} \\
\hline

RF-OB-34 & L'Utente deve poter modificare le informazioni della mail di avviso & \hyperref[sec:UC-17]{UC-17} \\
\hline

RF-OB-35 & L'Utente deve poter modificare il titolo della mail di avviso & \hyperref[sec:UC-17.1]{UC-17.1} \\
\hline

RF-OB-36 & L'Utente deve poter modificare il corpo della mail di avviso & \hyperref[sec:UC-17.2]{UC-17.2} \\
\hline

RF-OB-37 & L'Utente deve poter visualizzare un'anteprima della mail di avviso & \hyperref[sec:UC-18]{UC-18} \\
\hline

RF-OB-38 & L'Utente deve poter visualizzare l'anteprima del corpo della mail di avviso & \hyperref[subsec:UC-18.1]{UC-18.1} \\
\hline

RF-OB-39 & L'Utente deve poter visualizzare un'anteprima del titolo della mail di avviso & \hyperref[subsec:UC-18.2]{UC-18.2} \\
\hline

RF-OB-40 & L'Utente deve poter attivare il Dead Man's Switch & \hyperref[sec:UC-19]{UC-19} \\
\hline

RF-OB-41 & L'Utente deve poter disattivare la funzionalità di Dead Man's Switch & \hyperref[sec:UC-20]{UC-20} \\
\hline

RF-OB-42 & L'Utente deve poter impostare una password per il diario criptato & \hyperref[subsec:UC-21]{UC-21} \\
\hline
...
RF-OB-45 & L'Utente deve poter reinserire la password del diario criptato dopo la visualizzazione del messaggio di errore esplicativo & \hyperref[subsec:UC-22]{UC-22} \\
\hline

RF-OB-46 & L'Utente deve poter impostare una password per il diario fittizio & \hyperref[subsec:UC-23]{UC-23} \\
\hline
...
RF-OB-50 & L'Utente deve poter reinserire la password del diario fittizio dopo la visualizzazione del messaggio di errore esplicativo & \hyperref[subsec:UC-22]{UC-22}, \hyperref[subsec:UC-24]{UC-24} \\
\hline

RF-OB-51 & L'Utente deve poter inserire la password per accedere al diario desiderato & \hyperref[subsec:UC-25]{UC-25} \\
\hline

RF-OB-52 & L'Utente deve poter inserire la password per accedere al diario criptato & \hyperref[subsec:UC-25.1]{UC-25.1} \\
\hline

RF-OB-53 & L'Utente deve poter inserire la password per accedere al diario fittizio & \hyperref[subsec:UC-25.2]{UC-25.2} \\
\hline
...
RF-OB-55 & L'Utente deve poter reinserire la password di accesso al diario dopo la visualizzazione del messaggio di errore esplicativo & \hyperref[subsec:UC-27]{UC-27} \\
\hline

RF-OB-57 & L'Utente deve poter visualizzare il contenuto del diario criptato & \hyperref[subsec:UC-28]{UC-28} \\
\hline

RF-OB-58 & L'Utente deve poter visualizzare la lista delle note presenti nel diario criptato & \hyperref[subsec:UC-28.1]{UC-28.1} \\
\hline

RF-OB-59 & L'Utente deve poter visualizzare l'anteprima della singola nota all'interno della lista delle note del diario criptato & \hyperref[subsec:UC-28.1.1]{UC-28.1.1} \\
\hline

RF-OB-60 & L'Utente deve poter ricaricare la sezione delle note del diario criptato dopo la visualizzazione dell'errore di caricamento dei dati dal database & \hyperref[subsec:UC-28.1]{UC-28.1}, \hyperref[sec:UC-5]{UC-5} \\
\hline

RF-OB-61 & L'Utente deve poter creare una nuova nota vuota all'interno del diario criptato & \hyperref[subsec:UC-31]{UC-31} \\
\hline

RF-OB-62 & L'Utente deve poter eliminare una nota esistente dal diario criptato & \hyperref[subsec:UC-30]{UC-30} \\
\hline
...
RF-OB-64 & L'Utente deve poter confermare, con un'apposita voce, l'eliminazione della nota dal diario criptato quando il sistema ne richiede la conferma & \hyperref[subsec:UC-30.1]{UC-30.1} \\
\hline

RF-OB-65 & L'Utente deve poter visualizzare il contenuto completo di una nota selezionata dalla lista delle note del diario criptato & \hyperref[subsec:UC-32]{UC-32} \\
\hline

RF-OB-66 & L'Utente deve poter visualizzare il contenuto testuale della nota del diario criptato & \hyperref[subsec:UC-32.1]{UC-32.1} \\
\hline

RF-OB-67 & L'Utente deve poter visualizzare il contenuto multimediale della nota del diario criptato & \hyperref[subsec:UC-32.2]{UC-32.2} \\
\hline

RF-OB-68 & L'Utente deve poter visualizzare il contenuto audio presente all'interno della nota del diario criptato & \hyperref[subsec:UC-32.2.1]{UC-32.2.1} \\
\hline

RF-OB-69 & L'Utente deve poter visualizzare le immagini presenti all'interno della nota del diario criptato & \hyperref[subsec:UC-32.2.2]{UC-32.2.2} \\
\hline

RF-OB-70 & L'Utente deve poter inserire del contenuto testuale all'interno di una nota del diario criptato & \hyperref[subsec:UC-33]{UC-33} \\
\hline

RF-OB-71 & L'Utente deve poter inserire del contenuto multimediale all'interno di una nota del diario criptato & \hyperref[subsec:UC-34]{UC-34} \\
\hline

RF-OB-72 & L'Utente deve poter inserire un'immagine all'interno di una nota del diario criptato & \hyperref[subsec:UC-34.1]{UC-34.1} \\
\hline

RF-OB-73 & L'Utente deve poter inserire una traccia audio all'interno di una nota del diario criptato & \hyperref[subsec:UC-34.2]{UC-34.2} \\
\hline

RF-OB-74 & L'Utente deve poter modificare il contenuto testuale presente all'interno di una nota del diario criptato & \hyperref[subsec:UC-35]{UC-35} \\
\hline

RF-OB-75 & L'Utente deve poter rimuovere del contenuto multimediale presente all'interno di una nota del diario criptato & \hyperref[subsec:UC-36]{UC-36} \\
\hline

RF-OB-76 & L'Utente deve poter rimuovere un'immagine presente all'interno di una nota del diario criptato & \hyperref[subsec:UC-36.1]{UC-36.1} \\
\hline

RF-OB-77 & L'Utente deve poter rimuovere una traccia audio presente all'interno di una nota del diario criptato & \hyperref[subsec:UC-36.2]{UC-36.2} \\
\hline

RF-OB-78 & L'Utente deve poter visualizzare la schermata del diario fittizio & \hyperref[subsec:UC-29]{UC-29} \\
\hline

RF-OB-79 & L'Utente deve poter visualizzare la lista delle note presenti nel diario fittizio & \hyperref[subsec:UC-29.1]{UC-29.1} \\
\hline

RF-OB-80 & L'Utente deve poter visualizzare l'anteprima della singola nota all'interno della lista delle note del diario fittizio & \hyperref[subsec:UC-29.1.1]{UC-29.1.1} \\
\hline

RF-OB-81 & L'Utente deve poter ricaricare la sezione delle note del diario fittizio dopo la visualizzazione dell'errore di caricamento dei dati dal database & \hyperref[subsec:UC-29.1]{UC-29.1}, \hyperref[sec:UC-5]{UC-5} \\
\hline

RF-OB-82 & L'Utente deve poter eliminare una nota esistente dal diario fittizio & \hyperref[subsec:UC-37]{UC-37} \\
\hline
...
RF-OB-84 & L'Utente deve poter confermare, con un'apposita voce, l'eliminazione della nota dal diario fittizio quando il sistema ne richiede la conferma & \hyperref[subsec:UC-37.1]{UC-37.1} \\
\hline

RF-OB-85 & L'Utente deve poter creare una nuova nota vuota all'interno del diario fittizio & \hyperref[subsec:UC-38]{UC-38} \\
\hline

RF-OB-86 & L'Utente deve poter visualizzare il contenuto completo di una nota selezionata dalla lista delle note del diario fittizio & \hyperref[subsec:UC-39]{UC-39} \\
\hline

RF-OB-87 & L'Utente deve poter visualizzare il contenuto testuale della nota del diario fittizio & \hyperref[subsec:UC-39.1]{UC-39.1} \\
\hline

RF-OB-88 & L'Utente deve poter visualizzare il contenuto multimediale della nota del diario fittizio & \hyperref[subsec:UC-39.2]{UC-39.2} \\
\hline

RF-OB-89 & L'Utente deve poter visualizzare il contenuto audio presente all'interno della nota del diario fittizio & \hyperref[subsec:UC-39.2.1]{UC-39.2.1} \\
\hline

RF-OB-90 & L'Utente deve poter visualizzare le immagini presenti all'interno della nota del diario fittizio & \hyperref[subsec:UC-39.2.2]{UC-39.2.2} \\
\hline

RF-OB-91 & L'Utente deve poter inserire del contenuto testuale all'interno di una nota del diario fittizio & \hyperref[subsec:UC-40]{UC-40} \\
\hline

RF-OB-92 & L'Utente deve poter inserire del contenuto multimediale all'interno di una nota del diario fittizio & \hyperref[subsec:UC-41]{UC-41} \\
\hline

RF-OB-93 & L'Utente deve poter inserire un'immagine all'interno di una nota del diario fittizio & \hyperref[subsec:UC-41.1]{UC-41.1} \\
\hline

RF-OB-94 & L'Utente deve poter inserire una traccia audio all'interno di una nota del diario fittizio & \hyperref[subsec:UC-41.2]{UC-41.2} \\
\hline

RF-OB-95 & L'Utente deve poter rimuovere del contenuto multimediale presente all'interno di una nota del diario fittizio & \hyperref[subsec:UC-42]{UC-42} \\
\hline

RF-OB-96 & L'Utente deve poter rimuovere un'immagine presente all'interno di una nota del diario fittizio & \hyperref[subsec:UC-42.1]{UC-42.1} \\
\hline

RF-OB-97 & L'Utente deve poter rimuovere una traccia audio presente all'interno di una nota del diario fittizio & \hyperref[subsec:UC-42.2]{UC-42.2} \\
\hline

RF-OB-98 & L'Utente deve poter modificare il contenuto testuale presente all'interno di una nota del diario fittizio & \hyperref[subsec:UC-43]{UC-43} \\
\hline

RF-OB-99 & L'Utente deve poter inviare un messaggio di SOS ai contatti fidati tramite un'apposita voce & \hyperref[sec:UC-44]{UC-44} \\
\hline

RF-OB-100 & Il sistema deve notificare l'Utente dell'assenza di connessione internet durante il tentativo di invio del messaggio SOS e proporre l'invio di un segnale acustico & \hyperref[sec:UC-45]{UC-45} \\
\hline

RF-OB-101 & Il sistema deve notificare l'Utente della mancanza di contatti fidati configurati durante il tentativo di invio del messaggio SOS e proporre l'invio di un segnale acustico & \hyperref[sec:UC-46]{UC-46} \\
\hline

RF-OB-102 & L'Utente deve poter avviare un segnale acustico di emergenza indipendente dalle impostazioni di volume del dispositivo & \hyperref[sec:UC-47]{UC-47} \\
\hline

RF-OB-103 & L'Utente deve poter inviare un messaggio al chatbot e ricevere una risposta generata dall'LLM in base al contesto scelto & \hyperref[sec:UC-48]{UC-48} \\
\hline

RF-OB-105 & Il sistema deve notificare l'Utente della mancanza di connessione internet durante la consultazione del chatbot & \hyperref[sec:UC-50]{UC-50} \\
\hline

RF-OB-106 & L'Utente deve poter ripetere l'invio del messaggio o scriverne uno nuovo dopo la notifica di assenza di connessione internet & \hyperref[sec:UC-50]{UC-50} \\
\hline

RF-OB-107 & L'Utente deve poter cambiare il contesto della chat a "Detective delle Relazioni" & \hyperref[sec:UC-51]{UC-51} \\
\hline

RF-OB-108 & L'Utente deve poter cambiare il contesto della chat a "Specchio Intelligente" & \hyperref[sec:UC-52]{UC-52} \\
\hline

RF-OB-111 & L'Utente deve poter creare una nuova sessione di chat con il chatbot & \hyperref[subsec:UC-54]{UC-54} \\
\hline

RF-OB-112 & L'Utente deve poter eliminare una chat dalla cronologia delle chat & \hyperref[subsec:UC-55]{UC-55} \\
\hline
...
RF-OB-114 & L'Utente deve poter confermare, con un'apposita voce, l'eliminazione di una chat quando il sistema ne richiede la conferma & \hyperref[subsec:UC-55.1]{UC-55.1} \\
\hline

RF-OB-115 & L'Utente deve poter visualizzare il contenuto di una chat selezionata, inclusa la lista dei messaggi scambiati & \hyperref[subsec:UC-56]{UC-56} \\
\hline

RF-OB-116 & Il sistema deve notificare l'Utente di un errore nel caricamento del contenuto della chat & \hyperref[sec:UC-5]{UC-5}, \hyperref[subsec:UC-56]{UC-56} \\
\hline

RF-OB-117 & Il sistema deve visualizzare la lista dei messaggi scambiati tra l'Utente e il chatbot all'interno di una chat & \hyperref[subsec:UC-56.1]{UC-56.1} \\
\hline

RF-OB-118 & Il sistema deve distinguere visivamente i messaggi inviati dall'Utente da quelli generati dal chatbot & \hyperref[subsec:UC-56.1.1]{UC-56.1.1}, \hyperref[subsec:UC-56.1.2]{UC-56.1.2} \\
\hline

RF-OB-119 & Il sistema deve visualizzare i messaggi inviati dall'Utente con la formattazione grafica allineata a destra & \hyperref[subsec:UC-56.1.1]{UC-56.1.1} \\
\hline
...
RF-OB-121 & L'Utente deve poter visualizzare la cronologia delle chat con il chatbot & \hyperref[subsec:UC-57]{UC-57} \\
\hline

RF-OB-122 & L'Utente deve poter selezionare e visualizzare una singola chat dalla cronologia & \hyperref[subsec:UC-57.1]{UC-57.1} \\
\hline
...
RF-OB-124 & L'Utente deve poter visualizzare la lista delle community disponibili & \hyperref[sec:UC-58]{UC-58} \\
\hline

RF-OB-125 & Il sistema deve notificare l'Utente di un errore nel caricamento della lista delle community & \hyperref[sec:UC-5]{UC-5}, \hyperref[sec:UC-58]{UC-58} \\
\hline
...
RF-OB-129 & L'Utente deve poter aprire il link di una community per visitarne il sito web esterno & \hyperref[sec:UC-59]{UC-59} \\
\hline

RF-OB-130 & L'Utente deve poter visualizzare la lista delle leggi e normative disponibili & \hyperref[sec:UC-60]{UC-60} \\
\hline

RF-OB-131 & Il sistema deve mostrare il titolo di ogni singola legge nella lista & \hyperref[subsec:UC-60.1]{UC-60.1} \\
\hline

RF-OB-132 & L'Utente deve poter visualizzare i dettagli completi di una legge selezionata dalla lista & \hyperref[sec:UC-61]{UC-61} \\
\hline

RF-OB-133 & Il sistema deve notificare l'Utente di un errore nel caricamento dei dati di una legge & \hyperref[sec:UC-5]{UC-5}, \hyperref[sec:UC-61]{UC-61} \\
\hline

RF-OB-134 & L'Utente deve poter visualizzare la lista dei contatti e degli enti d'emergenza & \hyperref[sec:UC-62]{UC-62} \\
\hline
...
RF-OB-139 & L'Utente deve poter visualizzare la lista del materiale informativo disponibile & \hyperref[sec:UC-64]{UC-64} \\
\hline

RF-OB-140 & Il sistema deve notificare l'Utente di un errore nel caricamento della lista del materiale informativo & \hyperref[sec:UC-5]{UC-5}, \hyperref[sec:UC-64]{UC-64} \\
\hline
...
RF-OB-142 & L'Utente deve poter visualizzare il contenuto di un materiale informativo selezionato dalla lista & \hyperref[sec:UC-65]{UC-65} \\
\hline

RF-OB-143 & Il sistema deve notificare l'Utente di un errore nel recupero dei dati del materiale informativo & \hyperref[sec:UC-5]{UC-5}, \hyperref[sec:UC-65]{UC-65} \\
\hline

RF-OB-144 & L'Utente deve poter visualizzare la mappa interattiva con i marker dei luoghi sicuri nelle vicinanze & \hyperref[sec:UC-66]{UC-66} \\
\hline

RF-OB-145 & L'Utente deve poter aprire un luogo sicuro nella mappa esterna Google Maps & \hyperref[subsec:UC-67]{UC-67} \\
\hline

RF-OB-146 & Il sistema deve notificare l'Utente di un errore nell'apertura dell'applicazione Google Maps e mantenere la visualizzazione della mappa interna & \hyperref[subsec:UC-68]{UC-68} \\
\hline
\caption{Tabella Requisiti Funzionali obbligatori} \label{tab:requisiti_funzionali_obbligatori} \\
\end{longtable}
\end{center}
...
\rowcolor{white}
RF-DE-1 & Il sistema deve mostrare all'Utente una spiegazione della differenza tra "per me" e "per altri" & \hyperref[subsec:UC-3]{UC-3} \\
\hline
\rowcolor{gray!20}
RF-DE-2 & Il sistema mostra all'Utente una breve descrizione del funzionamento del Dead Man's Switch & \hyperref[sec:UC-19]{UC-19}, \hyperref[sec:UC-20]{UC-20} \\
\hline
\rowcolor{white}
RF-DE-3 & Il sistema deve permettere all'Utente di inserire più contenuti multimediali con una singola azione nel diario criptato & \hyperref[subsec:UC-34]{UC-34} \\
\hline
\rowcolor{gray!20}
RF-DE-4 & Il sistema deve permettere all'Utente di inserire più immagini con una singola azione nel diario criptato & \hyperref[subsec:UC-34.1]{UC-34.1} \\
\hline
\rowcolor{white}
RF-DE-5 & Il sistema deve permettere all'Utente di inserire più tracce audio con una singola azione nel diario criptato & \hyperref[subsec:UC-34.2]{UC-34.2} \\
\hline
\rowcolor{gray!20}
RF-DE-6 & Il sistema deve permettere all'Utente di inserire più contenuti multimediali con una singola azione nel diario fittizio & \hyperref[subsec:UC-41]{UC-41} \\
\hline
\rowcolor{white}
RF-DE-7 & Il sistema deve permettere all'Utente di inserire più immagini con una singola azione nel diario fittizio & \hyperref[subsec:UC-41.1]{UC-41.1} \\
\hline
\rowcolor{gray!20}
RF-DE-8 & Il sistema deve permettere all'Utente di inserire più tracce audio con una singola azione nel diario fittizio & \hyperref[subsec:UC-41.2]{UC-41.2} \\
\hline
\rowcolor{white}
RF-DE-9 & Il sistema deve permettere all'Utente di riprodurre le tracce audio inserite nel diario criptato & \hyperref[subsec:UC-32.2.1]{UC-32.2.1} \\
\hline
\rowcolor{gray!20}
RF-DE-10 & Il sistema deve permettere all'Utente di riprodurre le tracce audio inserite nel diario fittizio & \hyperref[subsec:UC-39.2.1]{UC-39.2.1} \\
\hline
\rowcolor{white}
RF-DE-11 & Il sistema deve indicare all'Utente che il messaggio del chatbot è in fase di caricamento & \hyperref[sec:UC-48]{UC-48} \\
\hline
\rowcolor{gray!20}
RF-DE-12 & Il sistema deve utilizzare i contenuti del diario criptato per personalizzare le risposte del chatbot & \hyperref[sec:UC-48]{UC-48} \\
\hline
\rowcolor{white}
RF-DE-13 & Il sistema deve mostrare all'Utente una spiegazione del tipo di contesto a cui può passare (Detective delle relazioni) & \hyperref[sec:UC-51]{UC-51} \\
\hline
\rowcolor{gray!20}
RF-DE-14 & Il sistema deve mostrare all'Utente una spiegazione del tipo di contesto a cui può passare (Specchio intelligente) & \hyperref[sec:UC-52]{UC-52} \\
\hline
\rowcolor{white}
RF-DE-15 & L'Utente deve poter comporre automaticamente il numero di un contatto d'emergenza tramite l'apposita voce & \hyperref[sec:UC-63]{UC-63} \\
\hline
...
\hline
\endfirsthead
\caption[]{Tabella Requisiti Funzionali opzionali (continua)} \\
\hline
\rowcolor{gray!60}
\textbf{Codice} & \textbf{Descrizione} & \textbf{Riferimento} \\
\hline
...
\rowcolor{gray!20}
RF-OP-1 & L'Utente deve poter impostare il timer di prima inattività (per la funzionalità di Dead Man's Switch) & \hyperref[sec:UC-15]{UC-15} \\
\hline
\rowcolor{white}
RF-OP-2 & L'Utente deve poter impostare il timer di seconda inattività (per la funzionalità di Dead Man's Switch) & \hyperref[sec:UC-16]{UC-16} \\
\hline
\rowcolor{gray!20}
RF-OP-3 & Il sistema deve inviare un SMS ai numeri di telefono dei contatti fidati all'invio del segnale SOS & \hyperref[sec:UC-44]{UC-44} \\
\hline
\rowcolor{white}
RF-OP-4 & Il sistema deve indicare all'Utente se le risorse sono autorevoli e verificate & \hyperref[sec:UC-59]{UC-59} \\
\hline
\rowcolor{gray!20}
RF-OP-5 & Il sistema deve mettere a disposizione leggi estere & \hyperref[sec:UC-60]{UC-60} \\
\hline
\rowcolor{white}
RF-OP-6 & Il sistema deve mostrare leggi aggiornate, giuridicamente valide e complete & \hyperref[sec:UC-61]{UC-61} \\
\hline
\rowcolor{gray!20}
RF-OP-7 & Il sistema deve indicare all'Utente se i luoghi sicuri visualizzati nella mappa sono autorevoli e verificati & \hyperref[sec:UC-66]{UC-66} \\
\hline
\rowcolor{white}
RF-OP-8 & L'Utente deve poter selezionare il profilo di utilizzo ("Per me" o "Per altri") & \hyperref[subsec:UC-3]{UC-3} \\
\hline
\rowcolor{gray!20}
RF-OP-9 & Il sistema deve bloccare temporaneamente l'accesso al diario in seguito al superamento del limite massimo di tentativi errati di inserimento password & \hyperref[subsec:UC-26]{UC-26} \\
\hline
\rowcolor{white}
RF-OP-10 & L'Utente deve poter interrompere la generazione della risposta dell'LLM durante la consultazione del chatbot & \hyperref[sec:UC-49]{UC-49} \\
\hline
\rowcolor{gray!20}
RF-OP-11 & Il sistema deve notificare l'Utente di un errore durante il cambio di contesto del chatbot e mantenere il contesto precedente & \hyperref[sec:UC-53]{UC-53} \\
\hline
\rowcolor{white}
RF-OP-12 & L'Utente deve poter ritentare il cambio di contesto del chatbot dopo la notifica di errore & \hyperref[sec:UC-53]{UC-53} \\
\hline

\caption{Tabella Requisiti Funzionali Opzionali} \label{tab:requisiti_funzionali_opzionali} \\
\end{longtable}
\end{center}
...
\endhead
\rowcolor{white}
RQ-OB-1 & Il Processo di sviluppo del software deve prevedere un'attività di Verifica continua tramite la stesura e l'esecuzione di Test di Unità per ogni componente implementato & Capitolato d'appalto \href{https://www.math.unipd.it/~tullio/IS-1/2025/Progetto/C4.pdf}{Capitolato} \\
\hline
\rowcolor{gray!20}
RQ-OB-2 & Le verifiche effettuate, le misurazioni sui test e i relativi risultati devono essere tracciati sistematicamente all'interno del documento Piano di qualifica & Decisione interna \href{https://swe-bitbybit.github.io/SWE-docs/docs/RTB/documenti_esterni/piano_di_qualifica.pdf}{Piano di Qualifica} \\
\hline
\rowcolor{white}
RQ-OB-3 & Tutte le attività del Ciclo di vita del prodotto (efficienza dei processi, convenzioni di codifica e stesura) devono conformarsi a quanto regolamentato dalle Norme di Progetto & Decisione interna \href{https://swe-bitbybit.github.io/SWE-docs/docs/RTB/documenti_interni/norme_di_progetto.pdf}{Norme di Progetto} \\
\hline
\rowcolor{gray!20}
RQ-OB-4 & Il Processo di tracciamento e versionamento per tutto il materiale di progetto, deve appoggiarsi al software Git e alla piattaforma GitHub & Decisione interna \href{https://swe-bitbybit.github.io/SWE-docs/docs/RTB/documenti_interni/norme_di_progetto.pdf}{Norme di Progetto} \\
\hline
\rowcolor{white}
RQ-OB-5 & Il monitoraggio della qualità del software deve basarsi sul calcolo strutturato di metriche statiche e dinamiche, le cui soglie sono stabilite nel Piano di qualifica & Decisione interna \href{https://swe-bitbybit.github.io/SWE-docs/docs/RTB/documenti_esterni/piano_di_qualifica.pdf}{Piano di Qualifica} \\
\hline
\rowcolor{gray!20}
RQ-OB-6 & Le procedure di controllo continuo sul materiale prodotto (come compilazione e analisi della qualità del codice) devono essere automatizzate mediante l'uso di una pipeline di Continuous Integration (GitHub Actions) & Decisione interna \href{https://swe-bitbybit.github.io/SWE-docs/docs/RTB/documenti_interni/norme_di_progetto.pdf}{Norme di Progetto} \\
\hline
\rowcolor{white}
RQ-OB-7 & L'assegnazione e l'avanzamento delle attività To-Do e la progressione complessiva del lavoro devono essere guidate da un'opportuna pianificazione basata su un sistema di Issue Tracking (GitHub Projects) & Decisione interna \href{https://swe-bitbybit.github.io/SWE-docs/docs/RTB/documenti_interni/norme_di_progetto.pdf}{Norme di Progetto} \\
\hline
\rowcolor{gray!20}
...
\hline
\rowcolor{white}
RV-OB-3 & Lo sviluppo dell'applicazione mobile e dell'interfaccia Utente deve essere condotto impiegando il Framework cross-platform Flutter & \href{https://www.math.unipd.it/~tullio/IS-1/2025/Progetto/C4.pdf}{Capitolato} \newline Sez. 3.4 \\
\hline
\rowcolor{gray!20}
...
\hline
\rowcolor{white}
RV-OB-5 & L'Architettura del Backend deve basarsi su un modello a microservizi, avvalendosi dell'esecuzione di funzioni AWS Lambda & \href{https://www.math.unipd.it/~tullio/IS-1/2025/Progetto/C4.pdf}{Capitolato} \newline Sez. 3.2 \\
\hline
\rowcolor{gray!20}
...
\hyperref[subsec:UC-3]{UC-3} & RF-OB-4, RF-DE-1 \\
\hline
...
\hyperref[subsec:UC-14]{UC-14} & RF-OB-32 \\
\hline

\hyperref[subsec:UC-13.1]{UC-13.1} & RF-OB-30, RF-OB-31 \\
\hline

\hyperref[sec:UC-15]{UC-15} & RF-OP-1 \\
\hline

\hyperref[sec:UC-16]{UC-16} & RF-OP-2 \\
\hline
...
\hyperref[sec:UC-19]{UC-19} & RF-OB-2, RF-OB-40, RF-DE-2 \\
\hline

\hyperref[sec:UC-20]{UC-20} & RF-OB-41, RF-DE-2 \\
\hline
...
\hyperref[subsec:UC-32.2.1]{UC-32.2.1} & RF-OB-68, RF-DE-9 \\
\hline
...
\hyperref[subsec:UC-34]{UC-34} & RF-OB-71, RF-DE-3 \\
\hline

\hyperref[subsec:UC-34.1]{UC-34.1} & RF-OB-72, RF-DE-4 \\
\hline

\hyperref[subsec:UC-34.2]{UC-34.2} & RF-OB-73, RF-DE-5 \\
\hline
...
\hyperref[subsec:UC-39.2.1]{UC-39.2.1} & RF-OB-89, RF-DE-10 \\
\hline
...
\hyperref[subsec:UC-41]{UC-41} & RF-OB-92, RF-DE-6 \\
\hline

\hyperref[subsec:UC-41.1]{UC-41.1} & RF-OB-93, RF-DE-7 \\
\hline

\hyperref[subsec:UC-41.2]{UC-41.2} & RF-OB-94, RF-DE-8 \\
\hline
...
\hyperref[sec:UC-44]{UC-44} & RF-OB-99, RF-OP-3 \\
\hline
...
\hyperref[sec:UC-48]{UC-48} & RF-OB-103, RF-DE-11, RF-DE-12 \\
\hline
...
\hyperref[sec:UC-51]{UC-51} & RF-OB-107, RF-DE-13 \\
\hline

\hyperref[sec:UC-52]{UC-52} & RF-OB-108, RF-DE-14 \\
\hline
...
\hyperref[sec:UC-59]{UC-59} & RF-OB-129, RF-OP-4 \\
\hline

\hyperref[sec:UC-60]{UC-60} & RF-OB-130, RF-OP-5 \\
\hline

\hyperref[subsec:UC-60.1]{UC-60.1} & RF-OB-131 \\
\hline

\hyperref[sec:UC-61]{UC-61} & RF-OB-132, RF-OB-133, RF-OP-6 \\
\hline
...
\hyperref[sec:UC-63]{UC-63} & RF-OB-138, RF-DE-15 \\
\hline
...
\hyperref[sec:UC-66]{UC-66} & RF-OB-144, RF-OP-7 \\
\hline
...
\subsection{Riepilogo dei requisiti} \label{subsection:riepilogo_requisiti}
...
\caption[]{Riepilogo dei requisiti (continua)} \\
\hline
\rowcolor{gray!60}
\textbf{Tipologia} & \textbf{Obbligatori} & \textbf{Desiderabili} & \textbf{Opzionali} \\
\hline
...
\caption{Riepilogo dei requisiti} \label{tab:riepilogo_requisiti} \\
\end{longtable}
\end{center}
\end{document}
