...
Il presente documento di \term{Analisi dei requisiti} ha l'obiettivo di esporre in modo esaustivo e non ambiguo le specifiche funzionali e vincolanti del sistema software oggetto di sviluppo.
...
RF-OB-2 & Il sistema deve attivare automaticamente la funzionalità di Dead Man's Switch a seguito del login dell'\term{Utente} & \hyperref[subsec:UC-1]{UC-1}, \hyperref[sec:UC-19]{UC-19} \\
\hline
...
RF-OB-32 & L'\term{Utente} deve poter effettuare il logout dopo essersi autenticato all'applicazione & \hyperref[subsec:UC-14]{UC-14} \\
\hline
...
\rowcolor{white}
RF-DE-1 & Il sistema deve mostrare all'\term{Utente} una spiegazione della differenza tra "per me" e "per altri" & \hyperref[subsec:UC-3]{UC-3} \\
\hline
\rowcolor{gray!20}
...
\hline
\rowcolor{gray!20}
RF-DE-4 & Il sistema deve permettere all'\term{Utente} di inserire più immagini con una singola azione nel diario criptato & \hyperref[subsec:UC-34.1]{UC-34.1} \\
\hline
\rowcolor{white}
RF-DE-5 & Il sistema deve permettere all'\term{Utente} di inserire più tracce audio con una singola azione nel diario criptato & \hyperref[subsec:UC-34.2]{UC-34.2} \\
\hline
\rowcolor{gray!20}
RF-DE-6 & Il sistema deve permettere all'\term{Utente} di inserire più contenuti multimediali con una singola azione nel diario fittizio & \hyperref[subsec:UC-41]{UC-41} \\
\hline
\rowcolor{white}
...
\hline
\rowcolor{gray!20}
RF-DE-8 & Il sistema deve permettere all'\term{Utente} di inserire più tracce audio con una singola azione nel diario fittizio & \hyperref[subsec:UC-41.2]{UC-41.2} \\
\hline
\rowcolor{white}
...
\hline
\rowcolor{gray!20}
RF-DE-10 & Il sistema deve permettere all'\term{Utente} di riprodurre le tracce audio inserite nel diario fittizio & \hyperref[subsec:UC-39.2.1]{UC-39.2.1} \\
\hline
\rowcolor{white}
RF-DE-11 & Il sistema deve indicare all'\term{Utente} che il messaggio del chatbot è in fase di caricamento & \hyperref[sec:UC-48]{UC-48} \\
\hline
\rowcolor{gray!20}
...
\hline
\rowcolor{white}
RF-DE-13 & Il sistema deve mostrare all'\term{Utente} una spiegazione del tipo di contesto a cui può passare (Detective delle relazioni) & \hyperref[sec:UC-51]{UC-51} \\
\hline
\rowcolor{gray!20}
RF-DE-14 & Il sistema deve mostrare all'\term{Utente} una spiegazione del tipo di contesto a cui può passare (Specchio intelligente) & \hyperref[sec:UC-52]{UC-52} \\
\hline
\rowcolor{white}
RF-DE-15 & L'\term{Utente} deve poter comporre automaticamente il numero di un contatto d'emergenza tramite l'apposita voce & \hyperref[sec:UC-63]{UC-63} \\
\hline
...
\rowcolor{gray!20}
RF-OP-1 & L'\term{Utente} deve poter impostare il timer di prima inattività (per la funzionalità di Dead Man's Switch) & \hyperref[sec:UC-15]{UC-15} \\
\hline
\rowcolor{white}
RF-OP-2 & L'\term{Utente} deve poter impostare il timer di seconda inattività (per la funzionalità di Dead Man's Switch) & \hyperref[sec:UC-16]{UC-16} \\
\hline
\rowcolor{gray!20}
...
\hline
\rowcolor{white}
RF-OP-4 & Il sistema deve indicare all'\term{Utente} se le risorse sono autorevoli e verificate & \hyperref[sec:UC-59]{UC-59} \\
\hline
\rowcolor{gray!20}
...
\hline
\rowcolor{white}
RV-OB-3 & Lo sviluppo dell'applicazione mobile e dell'interfaccia \term{Utente} deve essere condotto impiegando il \term{Framework} cross-platform Flutter & \href{https://www.math.unipd.it/~tullio/IS-1/2025/Progetto/C4.pdf}{Capitolato} \newline Sez. 3.4 \\
\hline
\rowcolor{gray!20}
RV-OB-4 & L'infrastruttura di \term{Backend} del sistema deve essere ospitata su servizi \term{Cloud} scalabili e sicuri, avvalendosi specificatamente del provider Amazon Web Services (AWS) & \href{https://www.math.unipd.it/~tullio/IS-1/2025/Progetto/C4.pdf}{Capitolato} \newline Sez. 3.4 \\
\hline
\rowcolor{white}
RV-OB-5 & L'\term{Architettura} del \term{Backend} deve basarsi su un modello a microservizi di tipo \term{Serverless}, avvalendosi dell'esecuzione di funzioni AWS Lambda & \href{https://www.math.unipd.it/~tullio/IS-1/2025/Progetto/C4.pdf}{Capitolato} \newline Sez. 3.2 \\
\hline
\rowcolor{gray!20}
RV-OB-6 & Le comunicazioni telematiche tra l'applicazione mobile (frontend) e i microservizi di \term{Backend} devono avvenire in modo sicuro tramite Amazon API Gateway & \href{https://www.math.unipd.it/~tullio/IS-1/2025/Progetto/C4.pdf}{Capitolato} \newline Sez. 3.4 \\
\hline
\rowcolor{white}
...
\hline
\rowcolor{white}
RV-OB-11 & L'infrastruttura \term{Cloud} del sistema deve essere configurata in modo da supportare l'alta disponibilità, delegando la resilienza dell'infrastruttura ai servizi gestiti AWS & Decisione interna; Decisione Esterna \\
\hline
