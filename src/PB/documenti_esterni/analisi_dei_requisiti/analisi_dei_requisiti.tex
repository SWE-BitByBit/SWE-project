...
Il sistema prevede un meccanismo di controllo denominato "Dead Man's Switch", che si attiva in caso di prolungata inattività dell'\term{Utente}. Se l'\term{Utente} non interagisce con l'app per un periodo di tempo configurabile, il sistema invierà una notifica all'\term{Utente} per controllare la propria situazione. In caso di mancata risposta, il sistema, dopo un ultieriore periodo di tempo, invierà una notifica ai contatti fidati, segnalando la potenziale situazione di rischio. Una qualsiasi interazione dell'\term{Utente} con l'app durante questo \term{Processo} annullerà le notifiche e resetterà il timer di controllo.
L'\term{Utente} avrà la possibilità di disattivare questa funzionalità in qualsiasi momento tramite le impostazioni.

\subsubsection{Luoghi Sicuri}
La sezione "Luoghi Sicuri" fornirà una mappa interattiva che evidenzia i luoghi considerati sicuri, come centri antiviolenza, stazioni di polizia, ospedali e altri punti di riferimento. L'\term{Utente} potrà visualizzareil luogo sicuro più vicino a sé. Questa funzionalità sarà integrata con Google Maps per garantire accuratezza e facilità d'uso.
...
\hline
\rowcolor{gray!20}
RF-OP-7 & Il sistema deve indicare all'\term{Utente} se i luoghi sicuri visualizzati nella mappa sono autorevoli e verificati & \hyperref[sec:UC-66]{UC-66} \\
\hline
\rowcolor{white}
RF-OP-8 & L'\term{Utente} deve poter selezionare il profilo di utilizzo ("Per me" o "Per altri") & \hyperref[subsec:UC-3]{UC-3} \\
\hline
\rowcolor{gray!20}
...
\hline
\rowcolor{white}
RF-OP-10 & L'\term{Utente} deve poter interrompere la generazione della risposta dell'LLM durante la consultazione del chatbot & \hyperref[sec:UC-49]{UC-49} \\
\hline
\rowcolor{gray!20}
RF-OP-11 & Il sistema deve notificare l'\term{Utente} di un errore durante il cambio di contesto del chatbot e mantenere il contesto precedente & \hyperref[sec:UC-53]{UC-53} \\
\hline
\rowcolor{white}
RF-OP-12 & L'\term{Utente} deve poter ritentare il cambio di contesto del chatbot dopo la notifica di errore & \hyperref[sec:UC-53]{UC-53} \\
\hline
